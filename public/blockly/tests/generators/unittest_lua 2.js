...
Blockly.Lua.unittest_main = function (block) {
  ...
  const resultsVar = Blockly.Lua.variableDB_.getName(
    "unittestResults",
    Blockly.Names.DEVELOPER_VARIABLE_TYPE
  );
  const functionName = Blockly.Lua.provideFunction_("unittest_report", [
...
Blockly.Lua.unittest_main.defineAssert_ = function (block) {
  ...
  const functionName = Blockly.Lua.provideFunction_("assertEquals", [
    "function " +
...
Blockly.Lua.unittest_assertequals = function (block) {
  // Asserts that a value equals another value.
  const message =
    Blockly.Lua.valueToCode(block, "MESSAGE", Blockly.Lua.ORDER_NONE) || "";
  const actual =
    Blockly.Lua.valueToCode(block, "ACTUAL", Blockly.Lua.ORDER_NONE) || "nil";
  const expected =
    Blockly.Lua.valueToCode(block, "EXPECTED", Blockly.Lua.ORDER_NONE) || "nil";
  return (
    Blockly.Lua.unittest_main.defineAssert_() +
    "(" +
    actual +
    ", " +
    expected +
...
Blockly.Lua.unittest_fail = function (block) {
  ...
  const message = Blockly.Lua.quote_(block.getFieldValue("MESSAGE"));
  const functionName = Blockly.Lua.provideFunction_("unittest_fail", [
...
Blockly.Lua.unittest_adjustindex = function (block) {
  const index =
    Blockly.Lua.valueToCode(block, "INDEX", Blockly.Lua.ORDER_ADDITIVE) || "0";
