...
Blockly.PHP.unittest_main = function (block) {
  ...
  const resultsVar = Blockly.PHP.variableDB_.getName(
    "unittestResults",
    Blockly.Names.DEVELOPER_VARIABLE_TYPE
  );
  const functionName = Blockly.PHP.provideFunction_("unittest_report", [
...
Blockly.PHP.unittest_main.defineAssert_ = function (block) {
  ...
  const functionName = Blockly.PHP.provideFunction_("assertEquals", [
    "function " +
...
Blockly.PHP.unittest_assertequals = function (block) {
  // Asserts that a value equals another value.
  const message =
    Blockly.PHP.valueToCode(block, "MESSAGE", Blockly.PHP.ORDER_NONE) || "";
  const actual =
    Blockly.PHP.valueToCode(block, "ACTUAL", Blockly.PHP.ORDER_COMMA) || "null";
  const expected =
    Blockly.PHP.valueToCode(block, "EXPECTED", Blockly.PHP.ORDER_COMMA) ||
    "null";
  return (
    Blockly.PHP.unittest_main.defineAssert_() +
    "(" +
    actual +
    ", " +
    expected +
...
Blockly.PHP.unittest_fail = function (block) {
  ...
  const message = Blockly.PHP.quote_(block.getFieldValue("MESSAGE"));
  const functionName = Blockly.PHP.provideFunction_("unittest_fail", [
    "function " + Blockly.PHP.FUNCTION_NAME_PLACEHOLDER_ + "($message) {",
    "  global " + resultsVar + ";",
    "  // Always assert an error.",
    "  if (!" + resultsVar + ") {",
...
Blockly.PHP.unittest_adjustindex = function (block) {
  let index =
    Blockly.PHP.valueToCode(block, "INDEX", Blockly.PHP.ORDER_ADDITION) || "0";
