...
suite("WorkspaceSvg", function () {
  ...
  test("Replacing shadow disposes svg", function () {
    ...
    const inputConnection = this.workspace.getTopBlocks()[0].getInput("NAME")
      .connection;
  ...
  suite("updateToolbox", function () {
    ...
    test("Passing in string as toolboxdef", function () {
      const parseToolboxFake = sinon.spy(
        Blockly.utils.toolbox,
        "parseToolboxTree"
      );
      this.workspace.updateToolbox(
        '<xml><category name="something"></category></xml>'
      );
  ...
  suite.skip("Viewport change events", function () {
    ...
    suite("scroll", function () {
      test("centerOnBlock", function () {
        ...
        block.render();
        runViewportEventTest(
          () => this.workspace.zoomToFit(block.id),
          this.eventsFireStub,
          this.changeListenerSpy,
          this.workspace,
          this.clock
    ...
    suite("Blocks triggering viewport changes", function () {
      test("block render that doesn't trigger scroll", function () {
        ...
        block.render();
        this.clock.runAll();
        assertEventNotFired(this.eventsFireStub, Blockly.Events.Ui, {
          element: "viewport",
        });
      });
      ...
      test.skip("domToWorkspace that doesn't trigger scroll", function () {
        ...
        assertEventNotFired(this.changeListenerSpy, Blockly.Events.Ui, {
          element: "viewport",
        });
      });
