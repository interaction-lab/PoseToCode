  ...
  suite("Positioning", function () {
    setup(function () {
      ...
      this.boundsStub = sinon
        .stub(Blockly.DropDownDiv, "getBoundsInfo_")
        .returns({
          left: 0,
          right: 100,
          top: 0,
      ...
      this.sizeStub = sinon.stub(Blockly.utils.style, "getSize").returns({
        width: 60,
        height: 60,
      });
    ...
    test("Below, in Bounds", function () {
      const metrics = Blockly.DropDownDiv.getPositionMetrics_(50, 0, 50, -10);
    ...
    test("Above, in Bounds", function () {
      const metrics = Blockly.DropDownDiv.getPositionMetrics_(50, 100, 50, 90);
    ...
    test("Below, out of Bounds", function () {
      const metrics = Blockly.DropDownDiv.getPositionMetrics_(50, 60, 50, 50);
      // "Above" in value actually means below in render.
