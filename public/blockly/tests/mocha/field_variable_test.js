...
suite("Variable Fields", function () {
  const FAKE_VARIABLE_NAME = "yertle";
  const FAKE_ID = "turtle";
  ...
  function initField(fieldVariable, workspace) {
    const mockBlock = getMockBlock(workspace);
  ...
  function createAndInitFieldJson(workspace, variableName) {
    return initField(
      Blockly.FieldVariable.fromJson({ variable: variableName }),
      workspace
    );
  }
  function assertValue(variableField, expectedName, opt_expectedId) {
    const actualName = variableField.getText();
    const actualId = variableField.getValue();
  ...
  setup(function () {
    ...
    sinon
      .stub(Blockly.Variables, "generateUniqueName")
  ...
  test("Dropdown contains variables", function () {
    ...
    const fieldVariable = createAndInitFieldConstructor(
      this.workspace,
      "name1"
    );
    ...
    const result_options = Blockly.FieldVariable.dropdownCreate.call(
      fieldVariable
    );
  ...
  suite("Constructor", function () {
    test("Null", function () {
      const variableField = createAndInitFieldConstructor(this.workspace, null);
      assertValueDefault(variableField);
    });
    test("Undefined", function () {
      const variableField = createAndInitFieldConstructor(
        this.workspace,
        undefined
      );
      assertValueDefault(variableField);
  ...
  suite("fromJson", function () {
    test("Null", function () {
      const variableField = createAndInitFieldJson(this.workspace, null);
      assertValueDefault(variableField);
    });
    test("Undefined", function () {
      const variableField = createAndInitFieldJson(this.workspace, undefined);
      assertValueDefault(variableField);
    });
    test("No Value Before InitModel", function () {
      const variableField = new Blockly.FieldVariable("name1");
      chai.assert.equal(variableField.getText(), "");
      chai.assert.isNull(variableField.getValue());
    });
    test("Given Variable Name", function () {
      const variableField = createAndInitFieldJson(this.workspace, "name1");
  ...
  suite("setValue", function () {
    ...
    test("New Variable ID", function () {
      ...
      const oldId = variableField.getValue();
  ...
  suite("Validators", function () {
    setup(function () {
      ...
      this.variableField = createAndInitFieldConstructor(
        this.workspace,
        "name1"
      );
    });
  ...
  suite("Get variable types", function () {
    ...
    test("variableTypes is null", function () {
      ...
      const fieldVariable = new Blockly.FieldVariable("name1");
      const mockBlock = getMockBlock(this.workspace);
      fieldVariable.setSourceBlock(mockBlock);
  ...
  suite("Default types", function () {
    ...
    test("Default type mismatch", function () {
      ...
      chai.assert.throws(function () {
        const _fieldVariable = new Blockly.FieldVariable(
          null,
          null,
          ["a"],
          "b"
    ...
    test("Default type mismatch with empty array", function () {
      // Invalid default type when creating a variable field.
      chai.assert.throws(function () {
        const _fieldVariable = new Blockly.FieldVariable(null, null, ["a"]);
      });
    });
  });
