...
Blockly.registry.Type.CONNECTION_CHECKER = new Blockly.registry.Type(
  "connectionChecker"
);
...
Blockly.registry.Type.FLYOUTS_VERTICAL_TOOLBOX = new Blockly.registry.Type(
  "flyoutsVerticalToolbox"
);

/** @type {!Blockly.registry.Type<Blockly.IFlyout>} */
Blockly.registry.Type.FLYOUTS_HORIZONTAL_TOOLBOX = new Blockly.registry.Type(
...
Blockly.registry.register = function (type, name, registryItem, opt_quiet) {
  if (
    (!(type instanceof Blockly.registry.Type) && typeof type !== "string") ||
    String(type).trim() == ""
  ) {
    throw Error(
      'Invalid type "' +
...
Blockly.registry.getClass = function (type, name) {
  return /** @type {?function(new:T, ...?)} */ (Blockly.registry.getItem_(
    type,
    name
  ));
};
