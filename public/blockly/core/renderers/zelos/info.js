    ...
    if (!this.outputConnection) {
      const topHeight =
        Math.max(
          ...
          Math.max(this.constants_.NOTCH_HEIGHT, this.constants_.CORNER_RADIUS)
...
Blockly.zelos.RenderInfo.prototype.getElemCenterline_ = function (row, elem) {
  if (
    row.hasStatement &&
    !Blockly.blockRendering.Types.isSpacer(elem) &&
...
Blockly.zelos.RenderInfo.prototype.finalizeHorizontalAlignment_ = function () {
  ...
  for (var i = 0, row; (row = this.rows[i]); i++) {
    ...
    row.elements.unshift(
      new Blockly.blockRendering.InRowSpacer(this.constants_, -leftNegPadding)
    );
    row.elements.push(
      new Blockly.blockRendering.InRowSpacer(this.constants_, -rightNegPadding)
    );
  }
...
Blockly.zelos.RenderInfo.prototype.getNegativeSpacing_ = function (elem) {
  ...
  const constants = /** @type {!Blockly.zelos.ConstantProvider} */ (this
    .constants_);
