...
Blockly.inject = function (container, opt_options) {
  ...
  const workspaceDragSurface = new Blockly.WorkspaceDragSurfaceSvg(
    subContainer
  );
...
Blockly.createMainWorkspace_ = function (
  ...
  if (!wsOptions.readOnly && !mainWorkspace.isMovable()) {
    ...
    const getWorkspaceMetrics = function () {
      const workspaceMetrics = Object.create(null);
      const defaultMetrics = mainWorkspace.getMetrics();
      ...
      if (mainWorkspace.isContentBounded()) {
        ...
        const blocksBoundingBox = mainWorkspace.getBlocksBoundingBox();
    ...
    const getObjectMetrics = function (object) {
    ...
    const bumpObjects = function (e) {
      ...
      if (
        ...
        if (
          ...
          if (object) {
            const objectMetrics = getObjectMetrics(object);
            ...
            const newYPosition = Blockly.utils.math.clamp(
              topClamp,
              objectMetrics.top,
              bottomClamp
            );
            ...
            let leftClamp = metrics.viewLeft;
            ...
            let rightClamp = metrics.viewRight - objectMetrics.width;
            if (metrics.RTL) {
            ...
            const newXPosition = Blockly.utils.math.clamp(
              leftClamp,
              objectMetrics.left,
              rightClamp
            );
...
Blockly.init_ = function (mainWorkspace) {
  ...
  let verticalSpacing = Blockly.Scrollbar.scrollbarThickness;
