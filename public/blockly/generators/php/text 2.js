...
Blockly.PHP.text_join = function (block) {
  ...
  if (block.itemCount_ == 0) {
    ...
    const element =
      Blockly.PHP.valueToCode(block, "ADD0", Blockly.PHP.ORDER_NONE) || "''";
    var code = element;
    return [code, Blockly.PHP.ORDER_FUNCTION_CALL];
  } else if (block.itemCount_ == 2) {
    const element0 =
      Blockly.PHP.valueToCode(block, "ADD0", Blockly.PHP.ORDER_ATOMIC) || "''";
    const element1 =
      Blockly.PHP.valueToCode(block, "ADD1", Blockly.PHP.ORDER_ATOMIC) || "''";
...
Blockly.PHP.text_append = function (block) {
  ...
  const value =
    Blockly.PHP.valueToCode(block, "TEXT", Blockly.PHP.ORDER_ASSIGNMENT) ||
    "''";
...
Blockly.PHP.text_charAt = function (block) {
  ...
  const textOrder =
    where == "RANDOM" ? Blockly.PHP.ORDER_NONE : Blockly.PHP.ORDER_COMMA;
  ...
  switch (where) {
    ...
    case "FROM_END":
      ...
      return [code, Blockly.PHP.ORDER_FUNCTION_CALL];
    case "RANDOM":
      var functionName = Blockly.PHP.provideFunction_("text_random_letter", [
...
Blockly.PHP.text_getSubstring = function (block) {
  // Get substring.
  const text =
    Blockly.PHP.valueToCode(block, "STRING", Blockly.PHP.ORDER_FUNCTION_CALL) ||
    "''";
  ...
  if (where1 == "FIRST" && where2 == "LAST") {
    ...
    const functionName = Blockly.PHP.provideFunction_("text_get_substring", [
      "function " +
        Blockly.PHP.FUNCTION_NAME_PLACEHOLDER_ +
...
Blockly.PHP.text_changeCase = function (block) {
  ...
  const text =
    Blockly.PHP.valueToCode(block, "TEXT", Blockly.PHP.ORDER_NONE) || "''";
...
Blockly.PHP.text_count = function (block) {
  const text =
    Blockly.PHP.valueToCode(block, "TEXT", Blockly.PHP.ORDER_MEMBER) || "''";
  const sub =
    Blockly.PHP.valueToCode(block, "SUB", Blockly.PHP.ORDER_NONE) || "''";
  const code =
    "strlen(" +
    sub +
    ") === 0" +
...
Blockly.PHP.text_replace = function (block) {
  const text =
    Blockly.PHP.valueToCode(block, "TEXT", Blockly.PHP.ORDER_MEMBER) || "''";
  const from =
    Blockly.PHP.valueToCode(block, "FROM", Blockly.PHP.ORDER_NONE) || "''";
  const to =
    Blockly.PHP.valueToCode(block, "TO", Blockly.PHP.ORDER_NONE) || "''";
