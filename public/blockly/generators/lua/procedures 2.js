...
Blockly.Lua.procedures_defreturn = function (block) {
  ...
  const funcName = Blockly.Lua.variableDB_.getName(
  ...
  let returnValue =
    Blockly.Lua.valueToCode(block, "RETURN", Blockly.Lua.ORDER_NONE) || "";
  let xfix2 = "";
  ...
  for (let i = 0; i < variables.length; i++) {
    args[i] = Blockly.Lua.variableDB_.getName(
      variables[i],
...
Blockly.Lua.procedures_callreturn = function (block) {
  ...
  for (let i = 0; i < variables.length; i++) {
    args[i] =
      Blockly.Lua.valueToCode(block, "ARG" + i, Blockly.Lua.ORDER_NONE) ||
      "nil";
  }
...
Blockly.Lua.procedures_ifreturn = function (block) {
  ...
  const condition =
    Blockly.Lua.valueToCode(block, "CONDITION", Blockly.Lua.ORDER_NONE) ||
    "false";
