...
Blockly.Python.controls_repeat_ext = function (block) {
  ...
  if (block.getField("TIMES")) {
    ...
    // External number.
    var repeats =
      Blockly.Python.valueToCode(block, "TIMES", Blockly.Python.ORDER_NONE) ||
      "0";
  }
...
Blockly.Python.controls_for = function (block) {
  // For loop.
  const variable0 = Blockly.Python.variableDB_.getName(
  ...
  let increment =
    Blockly.Python.valueToCode(block, "BY", Blockly.Python.ORDER_NONE) || "1";
  let branch = Blockly.Python.statementToCode(block, "DO");
  branch = Blockly.Python.addLoopTrap(branch, block) || Blockly.Python.PASS;
  ...
  if (
    ...
    const scrub = function (arg, suffix) {
      if (Blockly.isNumber(arg)) {
        ...
        // It's complicated.
        const varName = Blockly.Python.variableDB_.getDistinctName(
          variable0 + suffix,
          Blockly.VARIABLE_CATEGORY_NAME
        );
