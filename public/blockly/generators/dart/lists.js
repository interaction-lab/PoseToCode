...
Blockly.Dart.lists_create_with = function (block) {
  ...
  for (let i = 0; i < block.itemCount_; i++) {
    elements[i] =
      Blockly.Dart.valueToCode(block, "ADD" + i, Blockly.Dart.ORDER_NONE) ||
      "null";
  }
...
Blockly.Dart.lists_repeat = function (block) {
  ...
  const element =
    Blockly.Dart.valueToCode(block, "ITEM", Blockly.Dart.ORDER_NONE) || "null";
  const repeatCount =
...
Blockly.Dart.lists_indexOf = function (block) {
  ...
  const item =
    Blockly.Dart.valueToCode(block, "FIND", Blockly.Dart.ORDER_NONE) || "''";
  const list =
    Blockly.Dart.valueToCode(
      block,
      "VALUE",
...
Blockly.Dart.lists_getIndex = function (block) {
  ...
  function cacheList() {
    const listVar = Blockly.Dart.variableDB_.getDistinctName(
      "tmp_list",
  ...
  if (
    ...
    if (where == "RANDOM") {
      ...
      if (mode == "REMOVE") {
        // We can use multiple statements.
        var at = Blockly.Dart.getAdjusted(
          block,
          "AT",
          1,
          false,
        ...
        var functionName = Blockly.Dart.provideFunction_("lists_get_from_end", [
          "dynamic " +
...
Blockly.Dart.lists_setIndex = function (block) {
  ...
  const where = block.getFieldValue("WHERE") || "FROM_START";
  let list =
    Blockly.Dart.valueToCode(block, "LIST", Blockly.Dart.ORDER_UNARY_POSTFIX) ||
    "[]";
  const value =
    Blockly.Dart.valueToCode(block, "TO", Blockly.Dart.ORDER_ASSIGNMENT) ||
    "null";
  // Cache non-trivial values to variables to prevent repeated look-ups.
  // Closure, which accesses and modifies 'list'.
  function cacheList() {
  ...
  if (list.match(/^\w+$/) || (where1 != "FROM_END" && where2 == "FROM_START")) {
    ...
    switch (where1) {
      case "FROM_START":
        ...
        break;
      case "FROM_END":
        var at1 = Blockly.Dart.getAdjusted(
          block,
          "AT1",
          1,
          false,
    ...
    switch (where2) {
      case "FROM_START":
        ...
        break;
      case "FROM_END":
        var at2 = Blockly.Dart.getAdjusted(
          block,
          "AT2",
          0,
          false,
    ...
    const functionName = Blockly.Dart.provideFunction_("lists_get_sublist", [
      "List " +
        Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ +
...
Blockly.Dart.lists_sort = function (block) {
  ...
  const sortFunctionName = Blockly.Dart.provideFunction_("lists_sort", [
    "List " +
      Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ +
...
Blockly.Dart.lists_split = function (block) {
  ...
  let input = Blockly.Dart.valueToCode(
    block,
    "INPUT",
    Blockly.Dart.ORDER_UNARY_POSTFIX
  );
  const delimiter =
    Blockly.Dart.valueToCode(block, "DELIM", Blockly.Dart.ORDER_NONE) || "''";
