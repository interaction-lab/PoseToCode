...
Blockly.Dart.procedures_defreturn = function (block) {
  ...
  const funcName = Blockly.Dart.variableDB_.getName(
  ...
  let returnValue =
    Blockly.Dart.valueToCode(block, "RETURN", Blockly.Dart.ORDER_NONE) || "";
  let xfix2 = "";
  ...
  for (let i = 0; i < variables.length; i++) {
    args[i] = Blockly.Dart.variableDB_.getName(
      variables[i],
...
Blockly.Dart.procedures_callreturn = function (block) {
  ...
  for (let i = 0; i < variables.length; i++) {
    args[i] =
      Blockly.Dart.valueToCode(block, "ARG" + i, Blockly.Dart.ORDER_NONE) ||
      "null";
  }
...
Blockly.Dart.procedures_ifreturn = function (block) {
  ...
  const condition =
    Blockly.Dart.valueToCode(block, "CONDITION", Blockly.Dart.ORDER_NONE) ||
    "false";
  ...
  if (block.hasReturnValue_) {
    const value =
      Blockly.Dart.valueToCode(block, "VALUE", Blockly.Dart.ORDER_NONE) ||
      "null";
