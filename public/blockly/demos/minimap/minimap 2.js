...
Minimap.init = function (workspace, minimap) {
  ...
  this.workspace.scrollbar.vScroll.setHandlePosition = function (newPosition) {
    ...
    const absolutePosition = this.handlePosition_ / this.ratio;
  ...
  this.svg = Blockly.utils.dom.createSvgElement(
    "svg",
    {
...
Minimap.mousedown = function (e) {
  // Using Blockly.bindEvent_ to attach mouse/touch listeners.
  Minimap.mouseMoveBindData = Blockly.bindEvent_(
    document,
    "mousemove",
    null,
    Minimap.mousemove
  );
  Minimap.mouseUpBindData = Blockly.bindEvent_(
    document,
    "mouseup",
    null,
    Minimap.mouseup
...
Minimap.mirrorEvent = function (event) {
  ...
  const minimapEvent = Blockly.Events.fromJson(json, Minimap.minimap);
...
Minimap.setDraggerHeight = function () {
  const workspaceMetrics = Minimap.workspace.getMetrics();
  const draggerHeight =
    (workspaceMetrics.viewHeight / Minimap.workspace.scale) *
...
Minimap.setDraggerWidth = function () {
  const workspaceMetrics = Minimap.workspace.getMetrics();
  const draggerWidth =
    (workspaceMetrics.viewWidth / Minimap.workspace.scale) *
    Minimap.minimap.scale;
  // It's zero when first block is placed.
  if (draggerWidth == 0) {
    return;
...
Minimap.scaleMinimap = function () {
  const minimapBoundingBox = Minimap.minimap.getBlocksBoundingBox();
  const workspaceBoundingBox = Minimap.workspace.getBlocksBoundingBox();
  const workspaceMetrics = Minimap.workspace.getMetrics();
  const minimapMetrics = Minimap.minimap.getMetrics();
  ...
  // is implemented.
  let topPadding =
    (workspaceMetrics.viewHeight * Minimap.minimap.scale) /
    (2 * Minimap.workspace.scale);
  let sidePadding =
    (workspaceMetrics.viewWidth * Minimap.minimap.scale) /
    (2 * Minimap.workspace.scale);

  // If actual padding is more than half view ports height,
  // change it to actual padding.
  if (
    ((workspaceBoundingBox.y * Minimap.workspace.scale -
      workspaceMetrics.contentTop) *
      Minimap.minimap.scale) /
      Minimap.workspace.scale >
    topPadding
  ) {
    topPadding =
      ((workspaceBoundingBox.y * Minimap.workspace.scale -
  ...
  // change it to actual padding.
  if (
    ((workspaceBoundingBox.x * Minimap.workspace.scale -
  ...
  const scalex =
    (minimapMetrics.viewWidth - 2 * sidePadding) / minimapBoundingBox.width;
  const scaley =
    (minimapMetrics.viewHeight - 2 * topPadding) / minimapBoundingBox.height;
...
Minimap.updateMapDragger = function (e) {
  const y = e.clientY;
  const x = e.clientX;
  const draggerHeight = Minimap.mapDragger.getAttribute("height");
  const draggerWidth = Minimap.mapDragger.getAttribute("width");

  let finalY = y - Minimap.rect.top - draggerHeight / 2;
  let finalX = x - Minimap.rect.left - draggerWidth / 2;

  const maxValidY =
    (Minimap.workspace.getMetrics().contentHeight -
      Minimap.workspace.getMetrics().viewHeight) *
    Minimap.minimap.scale;
  const maxValidX =
    (Minimap.workspace.getMetrics().contentWidth -
      Minimap.workspace.getMetrics().viewWidth) *
    Minimap.minimap.scale;
  ...
  Minimap.workspace.scrollbar.vScroll.set(
    (finalY * Minimap.workspace.scale) / Minimap.minimap.scale
  );
  Minimap.workspace.scrollbar.hScroll.set(
    (finalX * Minimap.workspace.scale) / Minimap.minimap.scale
  );
...
Minimap.onScrollChange = function (position, horizontal) {
  if (!Minimap.disableScrollChange) {
    Minimap.mapDragger.setAttribute(
      horizontal ? "x" : "y",
