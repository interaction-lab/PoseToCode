...
function formatChange() {
  ...
  if (document.getElementById("format").value == "Manual") {
    ...
    const code = languagePre.textContent.trim();
  ...
  while (contentsBlock) {
    if (!contentsBlock.disabled && !contentsBlock.getInheritedDisabled()) {
      ...
      const check = JSON.parse(
        getOptTypesFrom(contentsBlock, "TYPE") || "null"
      );
      if (check) {
        input.check = check;
      }
  ...
  switch (rootBlock.getFieldValue("CONNECTIONS")) {
    case "LEFT":
      JS.output = JSON.parse(
        getOptTypesFrom(rootBlock, "OUTPUTTYPE") || "null"
      );
      break;
    case "BOTH":
      JS.previousStatement = JSON.parse(
        getOptTypesFrom(rootBlock, "TOPTYPE") || "null"
      );
      JS.nextStatement = JSON.parse(
        getOptTypesFrom(rootBlock, "BOTTOMTYPE") || "null"
      );
      break;
    case "TOP":
      JS.previousStatement = JSON.parse(
...
function formatJavaScript_(blockType, rootBlock) {
  ...
  while (contentsBlock) {
    if (!contentsBlock.disabled && !contentsBlock.getInheritedDisabled()) {
      ...
      const fields = getFieldsJs_(contentsBlock.getInputTargetBlock("FIELDS"));
      for (let i = 0; i < fields.length; i++) {
...
function connectionLineJs_(functionName, typeName) {
  let type = getOptTypesFrom(getRootBlock(), typeName);
  if (type) {
    type = ", " + type;
  } else {
    type = "";
      ...
      switch (block.type) {
        ...
        case "field_dropdown":
          ...
          for (let i = 0; i < block.optionCount_; i++) {
            options[i] =
              "[" +
              escapeString(block.getFieldValue("USER" + i)) +
              ", " +
...
function getOptTypesFrom(block, name) {
  const types = getTypesFrom_(block, name);
  if (types.length == 0) {
    return undefined;
...
function updatePreview() {
  ...
  try {
    ...
    const previewBlock = previewWorkspace.newBlock(blockType);
...
function init() {
  ...
  if ("BlocklyStorage" in window && window.location.hash.length > 1) {
    ...
    const xml =
      '<xml xmlns="https://developers.google.com/blockly/xml"><block type="factory_base" deletable="false" movable="false"></block></xml>';
  ...
  document.getElementById("format").addEventListener("change", formatChange);
