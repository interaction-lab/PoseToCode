...
WorkspaceFactoryInit.initColourPicker_ = function (controller) {
  ...
  const colours = [20, 65, 120, 160, 210, 230, 260, 290, 330, ""];
  ...
  // Convert to 2D array.
  const maxCols = Math.ceil(Math.sqrt(colours.length));
  const grid = [];
  let row = [];
  for (var i = 0; i < colours.length; i++) {
    row.push(colours[i]);
 ...
 * @private
 */
WorkspaceFactoryInit.assignWorkspaceFactoryClickHandlers_ = function (
  controller
) {
  // Import Custom Blocks button.
  document
    .getElementById("button_importBlocks")
    .addEventListener("click", function () {
      blocklyFactory.openModal("dropdownDiv_importBlocks");
    });
  document
    .getElementById("input_importBlocksJson")
  ...
  document
    .getElementById("input_importBlocksJs")
    .addEventListener("change", function () {
  ...
  document
    .getElementById("input_loadToolbox")
    .addEventListener("change", function () {
  ...
  document
    .getElementById("input_loadPreload")
    .addEventListener("change", function () {
      controller.importFile(
        event.target.files[0],
  ...
  document.getElementById("tab_toolbox").addEventListener("click", function () {
    controller.setMode(WorkspaceFactoryController.MODE_TOOLBOX);
  });
  document.getElementById("tab_preload").addEventListener("click", function () {
  ...
  document
    .getElementById("dropdown_newCategory")
    .addEventListener("click", function () {
      controller.addCategory();
      blocklyFactory.closeModal();
    });
  document
    .getElementById("dropdown_loadCategory")
    .addEventListener("click", function () {
      controller.loadCategory();
      blocklyFactory.closeModal();
    });
  document
    .getElementById("dropdown_separator")
    .addEventListener("click", function () {
      controller.addSeparator();
      blocklyFactory.closeModal();
    });
  document
    .getElementById("dropdown_loadStandardToolbox")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("button_editCategory")
    .addEventListener("click", function () {
      const selected = controller.model.getSelected();
  ...
  document
    .getElementById("categorySave")
    .addEventListener("click", function () {
      const name = document.getElementById("categoryName").value.trim();
      let colour = document.getElementById("categoryColour").value;
  ...
  document
    .getElementById("button_addShadow")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("button_removeShadow")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("button_optionsHelp")
    .addEventListener("click", function () {
      open(
  ...
  document
    .getElementById("button_standardOptions")
    .addEventListener("click", function () {
      controller.setStandardOptionsAndUpdate();
    });
};
 ...
 * @private
 */
WorkspaceFactoryInit.addWorkspaceFactoryEventListeners_ = function (
  controller
) {
  ...
  window.addEventListener("keydown", function (e) {
    ...
    if (
      !(
        controller.keyEventsEnabled &&
        controller.selectedMode == WorkspaceFactoryController.MODE_TOOLBOX
      )
    ) {
      return;
    }
  ...
  const isInvalidBlockPlacement = function (block) {
    return (
  ...
  controller.toolboxWorkspace.addChangeListener(function (e) {
    ...
    if (
      e.type == Blockly.Events.BLOCK_MOVE ||
      (e.type == Blockly.Events.UI && e.element == "selected")
    ) {
      ...
      // shadow block.
      if (selected) {
        const isShadow = controller.isUserGenShadowBlock(selected.id);
      ...
      if (
        selected != null &&
        selected.getSurroundParent() != null &&
        ...
        if (selected != null && isInvalidBlockPlacement(selected)) {
          ...
          if (!controller.isUserGenShadowBlock(selected.id)) {
            ...
            selected.setWarningText(
              "Only shadow blocks can be nested inside\n" +
                "other shadow blocks."
            );
            ...
            selected.setWarningText(
              "Shadow blocks must be nested inside other" + " blocks."
            );
          }
    ...
    if (e.type == Blockly.Events.BLOCK_CREATE) {
      ...
      const newBaseBlock = controller.toolboxWorkspace.getBlockById(e.blockId);
      const allNewBlocks = [];
      ...
      if (variableCreated && !controller.hasVariablesCategory()) {
        if (
          confirm(
            "Your new block has a variables field. To use this block " +
      ...
      if (procedureCreated && !controller.hasProceduresCategory()) {
        if (
          confirm(
            "Your new block is a function block. To use this block " +
...
WorkspaceFactoryInit.displayAddShadow_ = function (show) {
  document.getElementById("button_addShadow").style.display = show
    ? "inline-block"
    : "none";
};
...
WorkspaceFactoryInit.displayRemoveShadow_ = function (show) {
  document.getElementById("button_removeShadow").style.display = show
    ? "inline-block"
    : "none";
};
 ...
 * @private
 */
WorkspaceFactoryInit.addWorkspaceFactoryOptionsListeners_ = function (
  controller
) {
  // Checking the grid checkbox displays grid options.
  document
    .getElementById("option_grid_checkbox")
    .addEventListener("change", function (e) {
      document.getElementById(
        "grid_options"
      ).style.display = document.getElementById("option_grid_checkbox").checked
        ? "block"
        : "none";
    });
  ...
  document
    .getElementById("option_zoom_checkbox")
    .addEventListener("change", function (e) {
      document.getElementById(
        "zoom_options"
      ).style.display = document.getElementById("option_zoom_checkbox").checked
        ? "block"
        : "none";
    });
  ...
  document
    .getElementById("option_readOnly_checkbox")
    .addEventListener("change", function (e) {
  ...
  document
    .getElementById("option_infiniteBlocks_checkbox")
    .addEventListener("change", function (e) {
      document.getElementById(
        "maxBlockNumber_option"
      ).style.display = document.getElementById(
        "option_infiniteBlocks_checkbox"
      ).checked
        ? "none"
        : "block";
    });
