...
WorkspaceFactoryModel.prototype.hasCategoryByName = function (name) {
  for (let i = 0; i < this.toolboxList.length; i++) {
    if (
      this.toolboxList[i].type == ListElement.TYPE_CATEGORY &&
...
WorkspaceFactoryModel.prototype.addElementToList = function (element) {
  ...
  this.hasVariableCategory =
    element.custom == "VARIABLE" ? true : this.hasVariableCategory;
  this.hasProcedureCategory =
    element.custom == "PROCEDURE" ? true : this.hasProcedureCategory;
...
WorkspaceFactoryModel.prototype.deleteElementFromList = function (index) {
  ...
  this.hasVariableCategory =
    this.toolboxList[index].custom == "VARIABLE"
      ? false
      : this.hasVariableCategory;
  this.hasProcedureCategory =
    this.toolboxList[index].custom == "PROCEDURE"
      ? false
      : this.hasProcedureCategory;
  // Remove element.
 ...
 */
WorkspaceFactoryModel.prototype.moveElementToIndex = function (
  element,
  newIndex,
  oldIndex
) {
  ...
  if (
    newIndex < 0 ||
    newIndex >= this.toolboxList.length ||
    oldIndex < 0 ||
 ...
 *   loaded.
 */
WorkspaceFactoryModel.prototype.getShadowBlocksInWorkspace = function (
  workspaceBlocks
) {
  const shadowsInWorkspace = [];
...
WorkspaceFactoryModel.prototype.getAllUsedBlockTypes = function () {
  ...
  const pushBlockTypesToList = function (xml, list) {
...
WorkspaceFactoryModel.prototype.isDefinedBlockType = function (blockType) {
  ...
  const isLibBlock = this.libBlockTypes.indexOf(blockType) != -1;
  const isImportedBlock = this.importedBlockTypes.indexOf(blockType) != -1;
...
ListElement.prototype.saveFromWorkspace = function (workspace) {
  ...
  if (
    this.type == ListElement.TYPE_FLYOUT ||
