...
WorkspaceFactoryController.prototype.addCategory = function () {
  ...
  const isFirstCategory = !this.model.hasElements();
  // Get name from user.
  const name = this.promptForNewCategoryName(
...
WorkspaceFactoryController.prototype.createCategory = function (name) {
  ...
  // Create new category.
  const tab = this.view.addCategoryRow(name, category.id);
...
WorkspaceFactoryController.prototype.addClickToSwitch = function (tab, id) {
  const self = this;
  const clickFunction = function (id) {
    // Keep this in scope for switchElement.
    return function () {
...
WorkspaceFactoryController.prototype.removeElement = function () {
  ...
  const selectedIndex = this.model.getIndexByElementId(selectedId);
  ...
  let next = this.model.getElementByIndex(selectedIndex);
  ...
  if (!nextId) {
    alert(
      "You currently have no categories or separators. All your blocks" +
 ...
 */
WorkspaceFactoryController.prototype.promptForNewCategoryName = function (
  promptString,
  opt_oldName
) {
...
WorkspaceFactoryController.prototype.exportXmlFile = function (exportMode) {
  ...
  if (exportMode == WorkspaceFactoryController.MODE_TOOLBOX) {
    ...
    var fileName = prompt(
      "File Name for pre-loaded workspace XML:",
      "workspace.xml"
    );
  }
  if (!fileName) {
  ...
  if (exportMode == WorkspaceFactoryController.MODE_TOOLBOX) {
    ...
    const msg = "Unknown export mode: " + exportMode;
    BlocklyDevTools.Analytics.onError(msg);
    throw Error(msg);
  }

  // Download file.
  const data = new Blob([configXml], { type: "text/xml" });
  this.view.createAndDownloadFile(fileName, data);

  if (exportMode == WorkspaceFactoryController.MODE_TOOLBOX) {
    BlocklyDevTools.Analytics.onExport(BlocklyDevTools.Analytics.TOOLBOX, {
...
WorkspaceFactoryController.prototype.exportInjectFile = function () {
  const fileName = prompt(
    "File Name for starter Blockly workspace code:",
    "workspace.js"
  );
  if (!fileName) {
    // If cancelled.
  ...
  // necessary).
  this.generateNewOptions();
  const printableOptions = this.generator.generateInjectString();
  const data = new Blob([printableOptions], { type: "text/javascript" });
  this.view.createAndDownloadFile(fileName, data);

  BlocklyDevTools.Analytics.onExport(BlocklyDevTools.Analytics.STARTER_CODE, {
...
WorkspaceFactoryController.prototype.updatePreview = function () {
  ...
  if (!this.model.options.readOnly) {
    // Get toolbox XML.
    const tree = Blockly.utils.toolbox.parseToolboxTree(
      this.generator.generateToolboxXml()
    );
...
WorkspaceFactoryController.prototype.reinjectPreview = function (tree) {
  ...
  const injectOptions = this.readOptions_();
 ...
 */
WorkspaceFactoryController.prototype.changeSelectedCategory = function (
  name,
  colour
) {
...
WorkspaceFactoryController.prototype.moveElement = function (offset) {
  const curr = this.model.getSelected();
  if (!curr) {
    // Return if no selected element.
    return;
  }
  const currIndex = this.model.getIndexByElementId(curr.id);
  const swapIndex = this.model.getIndexByElementId(curr.id) + offset;
  const swap = this.model.getElementByIndex(swapIndex);
  if (!swap) {
    // Return if cannot swap in that direction.
    return;
  }
 ...
 */
WorkspaceFactoryController.prototype.moveElementToIndex = function (
  element,
  newIndex,
  oldIndex
) {
...
WorkspaceFactoryController.prototype.loadCategory = function () {
  ...
  do {
    var name = prompt(
      "Enter the name of the category you would like to import " +
...
WorkspaceFactoryController.prototype.loadCategoryByName = function (name) {
  ...
  if (this.model.hasVariables() && name.toLowerCase() == "variables") {
    alert(
      "A Variables category already exists. You cannot create multiple" +
  ...
  if (this.model.hasProcedures() && name.toLowerCase() == "functions") {
    alert(
      "A Functions category already exists. You cannot create multiple" +
...
WorkspaceFactoryController.prototype.importFile = function (file, importMode) {
  ...
  reader.onload = function () {
    ...
    try {
      const tree = Blockly.Xml.textToDom(reader.result);
      if (importMode == WorkspaceFactoryController.MODE_TOOLBOX) {
        ...
        const hasToolboxElements =
          controller.model.hasElements() ||
          controller.toolboxWorkspace.getAllBlocks(false).length > 0;
        if (hasToolboxElements) {
          var msg =
            "Are you sure you want to import? You will lose your " +
            "current toolbox.";
  ...
  if (tree.getElementsByTagName("category").length == 0) {
    ...
    for (var i = 0, item; (item = tree.children[i]); i++) {
      if (item.tagName == "category") {
        ...
        const category = this.model.getElementByIndex(i);
...
WorkspaceFactoryController.prototype.clearAll = function () {
  const msg =
    "Are you sure you want to clear all of your work in Workspace" +
    " Factory?";
  BlocklyDevTools.Analytics.onWarning(msg);
  if (!confirm(msg)) {
    return;
  }
  const hasCategories = this.model.hasElements();
  this.model.clearToolboxList();
  this.view.clearToolboxTabs();
 ...
 * @private
 */
WorkspaceFactoryController.prototype.addShadowForBlockAndChildren_ = function (
  block
) {
...
WorkspaceFactoryController.prototype.convertShadowBlocks = function () {
  const blocks = this.toolboxWorkspace.getAllBlocks(false);
...
WorkspaceFactoryController.prototype.setMode = function (mode) {
  ...
  if (mode == WorkspaceFactoryController.MODE_TOOLBOX) {
    ...
    this.clearAndLoadXml_(this.model.getSelectedXml());
    this.view.disableWorkspace(
      this.view.shouldDisableWorkspace(this.model.getSelected())
    );
  } else {
...
WorkspaceFactoryController.prototype.clearAndLoadXml_ = function (xml) {
  ...
  this.view.markShadowBlocks(
    this.model.getShadowBlocksInWorkspace(
...
WorkspaceFactoryController.prototype.generateNewOptions = function () {
  ...
  this.reinjectPreview(
    Blockly.utils.toolbox.parseToolboxTree(this.generator.generateToolboxXml())
  );
};
...
WorkspaceFactoryController.prototype.readOptions_ = function () {
  ...
  const readonly = document.getElementById("option_readOnly_checkbox").checked;
  if (readonly) {
    optionsObj.readOnly = true;
  } else {
    optionsObj.collapse = document.getElementById(
      "option_collapse_checkbox"
    ).checked;
    optionsObj.comments = document.getElementById(
      "option_comments_checkbox"
    ).checked;
    optionsObj.disable = document.getElementById(
      "option_disable_checkbox"
    ).checked;
    if (document.getElementById("option_infiniteBlocks_checkbox").checked) {
      optionsObj.maxBlocks = Infinity;
    } else {
      const maxBlocksValue = document.getElementById("option_maxBlocks_number")
        .value;
      optionsObj.maxBlocks =
        typeof maxBlocksValue === "string"
          ? parseInt(maxBlocksValue)
          : maxBlocksValue;
    }
    optionsObj.trashcan = document.getElementById(
      "option_trashcan_checkbox"
    ).checked;
    optionsObj.horizontalLayout = document.getElementById(
      "option_horizontalLayout_checkbox"
    ).checked;
    optionsObj.toolboxPosition = document.getElementById(
      "option_toolboxPosition_checkbox"
    ).checked
      ? "end"
      : "start";
  }
  ...
  optionsObj.scrollbars = document.getElementById(
    "option_scrollbars_checkbox"
  ).checked;
  optionsObj.sounds = document.getElementById("option_sounds_checkbox").checked;
  optionsObj.oneBasedIndex = document.getElementById(
    "option_oneBasedIndex_checkbox"
  ).checked;

  // If using a grid, add all grid options.
  if (document.getElementById("option_grid_checkbox").checked) {
    const grid = Object.create(null);
    const spacingValue = document.getElementById("gridOption_spacing_number")
      .value;
    grid.spacing =
      typeof spacingValue === "string" ? parseInt(spacingValue) : spacingValue;
    const lengthValue = document.getElementById("gridOption_length_number")
      .value;
    grid.length =
      typeof lengthValue === "string" ? parseInt(lengthValue) : lengthValue;
  ...
  if (document.getElementById("option_zoom_checkbox").checked) {
    ...
    zoom.controls = document.getElementById(
      "zoomOption_controls_checkbox"
    ).checked;
    zoom.wheel = document.getElementById("zoomOption_wheel_checkbox").checked;
    const startScaleValue = document.getElementById(
      "zoomOption_startScale_number"
    ).value;
    zoom.startScale =
      typeof startScaleValue === "string"
        ? Number(startScaleValue)
        : startScaleValue;
    const maxScaleValue = document.getElementById("zoomOption_maxScale_number")
      .value;
    zoom.maxScale =
      typeof maxScaleValue === "string" ? Number(maxScaleValue) : maxScaleValue;
    const minScaleValue = document.getElementById("zoomOption_minScale_number")
      .value;
    zoom.minScale =
      typeof minScaleValue === "string" ? Number(minScaleValue) : minScaleValue;
    const scaleSpeedValue = document.getElementById(
      "zoomOption_scaleSpeed_number"
    ).value;
    zoom.scaleSpeed =
      typeof scaleSpeedValue === "string"
        ? Number(scaleSpeedValue)
        : scaleSpeedValue;
...
WorkspaceFactoryController.prototype.importBlocks = function (file, format) {
  ...
  reader.onload = function () {
    try {
      ...
      const blockTypes = FactoryUtils.defineAndGetBlockTypes(
        reader.result,
        format
      );
      ...
      if (controller.model.hasDefinedBlockTypes(blockTypes)) {
        var msg =
          "An imported block uses the same name as a block " +
      ...
      const blocks = controller.generator.getDefinedBlocks(blockTypes);
      // Generate category XML and append to toolbox.
      const categoryXml = FactoryUtils.generateCategoryXml(
        blocks,
        categoryName
      );
      ...
      BlocklyDevTools.Analytics.onImport(
        "BlockDefinitions" + (format == "JSON" ? ".json" : ".js")
      );
    } catch (e) {
 ...
 *    library.
 */
WorkspaceFactoryController.prototype.setBlockLibCategory = function (
  categoryXml,
  libBlockTypes
) {
  const blockLibCategory = document.getElementById("blockLibCategory");
