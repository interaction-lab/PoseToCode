...
BlockFactory.formatChange = function () {
  ...
  if (
    document.getElementById("format").value == "Manual-JSON" ||
...
BlockFactory.updateLanguage = function () {
  ...
  if (!BlockFactory.updateBlocksFlag) {
    ...
    let code = FactoryUtils.getBlockDefinition(
      blockType,
      rootBlock,
      format,
...
BlockFactory.updatePreview = function () {
  ...
  const format = BlockFactory.getBlockDefinitionFormat();
  const code = document.getElementById("languageTA").value;
  if (!code.trim()) {
  ...
  try {
    ...
    for (const prop in backupBlocks) {
    ...
    if (format == "JSON") {
      ...
      try {
        ...
        console.error(
          "Error while evaluating JavaScript formatted block definition",
          e
        );
        return;
      }
    ...
    const previewBlock = BlockFactory.previewWorkspace.newBlock(blockType);
    ...
    if (StandardCategories.coreBlockTypes.indexOf(blockType) != -1) {
      rootBlock.setWarningText(
        "A core Blockly block already exists " + "under this name."
      );
    } else if (blockType == "block_type") {
      // Warn user to let them know they can't save a block under the default
      // name 'block_type'
      rootBlock.setWarningText(
        "You cannot save a block with the default " + 'name, "block_type"'
      );
    } else {
...
BlockFactory.disableEnableLink = function () {
  ...
  const saveBlockButton = document.getElementById("localSaveButton");
  const saveToLibButton = document.getElementById("saveToBlockLibraryButton");
  const disabled =
    document.getElementById("format").value.substr(0, 6) == "Manual";
...
BlockFactory.showStarterBlock = function () {
  ...
  const xml = Blockly.Xml.textToDom(BlockFactory.STARTER_BLOCK_XML_TEXT);
