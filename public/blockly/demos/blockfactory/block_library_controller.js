...
BlockLibraryController.prototype.getCurrentBlockType = function () {
  const rootBlock = FactoryUtils.getRootBlock(BlockFactory.mainWorkspace);
  const blockType = rootBlock.getFieldValue("NAME").trim().toLowerCase();
...
BlockLibraryController.prototype.openBlock = function (blockType) {
  if (blockType) {
    const xml = this.storage.getBlockXml(blockType);
...
BlockLibraryController.prototype.clearBlockLibrary = function () {
  const msg = "Delete all blocks from library?";
...
BlockLibraryController.prototype.saveToBlockLibrary = function () {
  ...
  if (blockType == "block_type") {
    ...
    const msg =
      'You cannot save a block under the name "block_type". Try ' +
  ...
  const block = FactoryUtils.getRootBlock(BlockFactory.mainWorkspace);
...
BlockLibraryController.prototype.has = function (blockType) {
  const blockLibrary = this.storage.blocks;
...
BlockLibraryController.prototype.populateBlockLibrary = function () {
  ...
  const blockLibrary = this.storage.blocks;
  for (const blockType in blockLibrary) {
 ...
 */
BlockLibraryController.prototype.setBlockLibraryStorage = function (
  blockLibStorage
) {
...
BlockLibraryController.prototype.warnIfUnsavedChanges = function () {
  if (!FactoryUtils.savedBlockChanges(this)) {
    return confirm(
      "You have unsaved changes. By proceeding without saving " +
...
BlockLibraryController.prototype.addOptionSelectHandler = function (blockType) {
  ...
  const setSelectedAndOpen_ = function (blockOption) {
    const blockType = blockOption.textContent;
  ...
  const blockOption = this.view.optionMap[blockType];
  // Use an additional closure to correctly assign the tab callback.
  blockOption.addEventListener("click", makeOptionSelectHandler_(blockOption));
};
...
BlockLibraryController.prototype.addOptionSelectHandlers = function () {
  ...
  for (const blockType in this.view.optionMap) {
    this.addOptionSelectHandler(blockType);
  }
};
...
BlockLibraryController.prototype.updateButtons = function (savedChanges) {
  const blockType = this.getCurrentBlockType();
  const isInLibrary = this.has(blockType);
