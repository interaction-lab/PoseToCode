...
Plane.getNumberParamFromUrl = function (name, minValue, maxValue) {
  const val = Number(Plane.getStringParamFromUrl(name, "NaN"));
...
Plane.getLang = function () {
  let lang = Plane.getStringParamFromUrl("lang", "");
...
Plane.changeLanguage = function () {
  ...
  if (typeof Blockly !== "undefined" && window.sessionStorage) {
    const xml = Blockly.Xml.workspaceToDom(Plane.workspace);
...
Plane.sliderChange = function (value) {
  const newRows = Math.round((value * 410) / 20);
...
Plane.setCorrect = function (ok) {
  const yes = document.getElementById("seatYes");
  const no = document.getElementById("seatNo");
...
Plane.recalculate = function () {
  ...
  let rootBlock = null;
  const blocks = Plane.workspace.getTopBlocks(false);
  for (var i = 0, block; (block = blocks[i]); i++) {
  ...
  const code = Blockly.JavaScript.blockToCode(rootBlock);
  try {
    seats = eval(code);
  } catch (e) {
...
Plane.redraw = function (newRows) {
  let rows1st = Plane.rows1st;
  let rows2nd = Plane.rows2nd;
  const svg = document.getElementById("plane");
  if (newRows != rows1st) {
