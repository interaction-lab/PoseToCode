...
Minimap.setDraggerHeight = function () {
  ...
  const draggerHeight = (workspaceMetrics.viewHeight / Minimap.workspace.scale) *
...
Minimap.setDraggerWidth = function () {
  const workspaceMetrics = Minimap.workspace.getMetrics()
  const draggerWidth = (workspaceMetrics.viewWidth / Minimap.workspace.scale) *
      Minimap.minimap.scale
  // It's zero when first block is placed.
  if (draggerWidth == 0) {
    return
...
Minimap.scaleMinimap = function () {
  ...
  // is implemented.
  let topPadding = (workspaceMetrics.viewHeight) * Minimap.minimap.scale /
      (2 * Minimap.workspace.scale)
  let sidePadding = (workspaceMetrics.viewWidth) * Minimap.minimap.scale /
      (2 * Minimap.workspace.scale)
  ...
  const scalex = (minimapMetrics.viewWidth - 2 * sidePadding) /
      minimapBoundingBox.width
  const scaley = (minimapMetrics.viewHeight - 2 * topPadding) /
...
Minimap.updateMapDragger = function (e) {
  ...
  const maxValidY = (Minimap.workspace.getMetrics().contentHeight -
      Minimap.workspace.getMetrics().viewHeight) * Minimap.minimap.scale
  const maxValidX = (Minimap.workspace.getMetrics().contentWidth -
