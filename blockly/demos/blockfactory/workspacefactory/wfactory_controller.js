...
WorkspaceFactoryController.prototype.addClickToSwitch = function (tab, id) {
  const self = this
  const clickFunction = function (id) { // Keep this in scope for switchElement.
    return function () {
  ...
  reader.onload = function () {
    ...
    try {
      ...
      if (importMode == WorkspaceFactoryController.MODE_TOOLBOX) {
        ...
        const hasToolboxElements = controller.model.hasElements() ||
...
WorkspaceFactoryController.prototype.clearAll = function () {
  const msg = 'Are you sure you want to clear all of your work in Workspace' +
      ' Factory?'
  BlocklyDevTools.Analytics.onWarning(msg)
  if (!confirm(msg)) {
    return
...
WorkspaceFactoryController.prototype.readOptions_ = function () {
  ...
  if (document.getElementById('option_grid_checkbox').checked) {
    ...
    grid.spacing = typeof spacingValue === 'string'
      ? parseInt(spacingValue)
      : spacingValue
    const lengthValue = document.getElementById('gridOption_length_number').value
    grid.length = typeof lengthValue === 'string'
      ? parseInt(lengthValue)
      : lengthValue
...
WorkspaceFactoryController.prototype.importBlocks = function (file, format) {
  ...
  reader.onload = function () {
    try {
      ...
      const blockTypes = FactoryUtils.defineAndGetBlockTypes(reader.result,
        format)
      ...
      const categoryXml = FactoryUtils.generateCategoryXml(blocks, categoryName)
