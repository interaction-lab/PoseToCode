...
AppController = function () {
  ...
  this.blockLibraryController = new BlockLibraryController(
    this.blockLibraryName
  );
  ...
  this.workspaceFactoryController = new WorkspaceFactoryController(
    "workspacefactory_toolbox",
    "toolbox_blocks",
    "preview_blocks"
  );

  // Initialize Block Exporter
  this.exporter = new BlockExporterController(
    this.blockLibraryController.storage
  );

  // Map of tab type to the div element for the tab.
  this.tabMap = Object.create(null);
  this.tabMap[AppController.BLOCK_FACTORY] = document.getElementById(
    "blockFactory_tab"
  );
  this.tabMap[AppController.WORKSPACE_FACTORY] = document.getElementById(
    "workspaceFactory_tab"
  );
  this.tabMap[AppController.EXPORTER] = document.getElementById(
    "blocklibraryExporter_tab"
  );
...
AppController.prototype.exportBlockLibraryToFile = function () {
  ...
  const blockLib = this.blockLibraryController.getBlockLibrary();
  // Concatenate the XMLs, each separated by a blank line.
  const blockLibText = this.formatBlockLibraryForExport_(blockLib);
  // Get file name.
  const filename = prompt(
    "Enter the file name under which to save your block " + "library.",
    "library.xml"
  );
  ...
  if (filename) {
    ...
    const msg =
      "Could not export Block Library without file name under which " +
      "to save library.";
...
AppController.prototype.formatBlockLibraryForExport_ = function (blockXmlMap) {
  ...
  for (const blockType in blockXmlMap) {
    const blockXmlDom = Blockly.Xml.textToDom(blockXmlMap[blockType]);
    const blockNode = blockXmlDom.firstElementChild;
...
AppController.prototype.formatBlockLibraryForImport_ = function (xmlText) {
  const inputXml = Blockly.Xml.textToDom(xmlText);
  ...
  const inputChildren = Array.from(inputXml.children);
  ...
  const blockXmlTextMap = Object.create(null);
  ...
  for (var i = 0, blockNode; (blockNode = inputChildren[i]); i++) {
    ...
    // main workspace.
    // Create DOM for XML.
    const editorWorkspaceXml = Blockly.utils.xml.createElement("xml");
    ...
    let blockType = this.getBlockTypeFromXml_(xmlText).toLowerCase();
...
AppController.prototype.getBlockTypeFromXml_ = function (xmlText) {
  ...
  const factoryBaseBlockXml = xmlDom.getElementsByTagName("block")[0];
  // Get field elements from factory base.
  const fields = factoryBaseBlockXml.getElementsByTagName("field");
...
AppController.prototype.onTab = function () {
  ...
  if (
    ...
    const hasUnsavedChanges = !FactoryUtils.savedBlockChanges(
      this.blockLibraryController
    );
    if (hasUnsavedChanges) {
      const msg = "You have unsaved changes in Block Factory.";
  ...
  if (this.selectedTab == AppController.EXPORTER) {
    ...
    const usedBlockTypes = this.workspaceFactoryController.getAllUsedBlockTypes();
    ...
    const categoryXml = this.exporter.getBlockLibraryCategory();
    const blockTypes = this.blockLibraryController.getStoredBlockTypes();
    this.workspaceFactoryController.setBlockLibCategory(
      categoryXml,
      blockTypes
    );
  }
...
AppController.prototype.assignExporterClickHandlers = function () {
  const self = this;
  document
    .getElementById("button_setBlocks")
    .addEventListener("click", function () {
      self.openModal("dropdownDiv_setBlocks");
    });

  document
    .getElementById("dropdown_addAllUsed")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("dropdown_addAllFromLib")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("clearSelectedButton")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("exporterSubmitButton")
    .addEventListener("click", function () {
...
AppController.prototype.assignExporterChangeListeners = function () {
  ...
  const blockDefCheck = document.getElementById("blockDefCheck");
  const genStubCheck = document.getElementById("genStubCheck");
...
AppController.prototype.assignLibraryClickHandlers = function () {
  ...
  document
    .getElementById("saveToBlockLibraryButton")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("removeBlockFromLibraryButton")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("clearBlockLibraryButton")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("button_blockLib")
    .addEventListener("click", function () {
...
AppController.prototype.assignBlockFactoryClickHandlers = function () {
  ...
  document
    .getElementById("localSaveButton")
    .addEventListener("click", function () {
  ...
  document.getElementById("files").addEventListener("change", function () {
    ...
    const replace = confirm(
      "This imported block library will " +
  ...
  document
    .getElementById("createNewBlockButton")
    .addEventListener("click", function () {
      // If there are unsaved changes warn user, check if they'd like to
      // proceed with unsaved changes, and act accordingly.
      const proceedWithUnsavedChanges = self.blockLibraryController.warnIfUnsavedChanges();
...
AppController.prototype.init = function () {
  ...
  // Hide and show the block library dropdown.
  document.getElementById("modalShadow").addEventListener("click", function () {
    self.closeModal();
  });
  ...
  const toolbox = document.getElementById("blockfactory_toolbox");
