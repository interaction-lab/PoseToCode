...
BlockExporterController.prototype.export = function () {
  ...
  if (wantBlockDef) {
    ...
    if (!blockDef_filename) {
      ...
      const blockDefs = this.tools.getBlockDefinitions(blockXmlMap,
        definitionFormat)
  ...
  if (wantGenStub) {
    ...
    if (!generatorStub_filename) {
      ...
      const genStubs = this.tools.getGeneratorCode(blockXmlMap,
        language)
...
BlockExporterController.prototype.selectUsedBlocks = function () {
  ...
  if (unstoredCustomBlockTypes.length > 0) {
    ...
    const customWarning = 'Custom blocks used in workspace factory but not ' +
        'stored in block library:\n ' + blockTypesText +
