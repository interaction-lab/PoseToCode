 ...
 */
BlockExporterController.prototype.setBlockLibraryStorage = function (
  blockLibStorage
) {
  this.blockLibStorage = blockLibStorage;
};
 ...
 *    that stores the blocks.
 */
BlockExporterController.prototype.getBlockLibraryStorage = function (
  blockLibStorage
) {
...
BlockExporterController.prototype.export = function () {
  ...
  const blockTypes = this.view.getSelectedBlockTypes();
  const blockXmlMap = this.blockLibStorage.getBlockXmlMap(blockTypes);

  // Pull block definition(s) settings from the Export Settings form.
  const wantBlockDef = document.getElementById("blockDefCheck").checked;
  const definitionFormat = document.getElementById("exportFormat").value;
  const blockDef_filename = document.getElementById("blockDef_filename").value;

  // Pull block generator stub(s) settings from the Export Settings form.
  const wantGenStub = document.getElementById("genStubCheck").checked;
  const language = document.getElementById("exportLanguage").value;
  const generatorStub_filename = document.getElementById(
    "generatorStub_filename"
  ).value;
  ...
  if (wantBlockDef) {
    ...
    if (!blockDef_filename) {
      ...
      var msg =
        "Please enter a filename for your block definition(s) download.";
      BlocklyDevTools.Analytics.onWarning(msg);
      alert(msg);
    } else {
      // Get block definition code in the selected format for the blocks.
      const blockDefs = this.tools.getBlockDefinitions(
        blockXmlMap,
        definitionFormat
      );
  ...
  if (wantGenStub) {
    ...
    if (!generatorStub_filename) {
      ...
      const genStubs = this.tools.getGeneratorCode(blockXmlMap, language);

      // Download the file.
      FactoryUtils.createAndDownloadFile(
        genStubs,
        generatorStub_filename + ".js",
        "javascript"
      );
...
BlockExporterController.prototype.updateSelector = function () {
  ...
  const oldSelectedTypes = this.view.getSelectedBlockTypes();
...
BlockExporterController.prototype.selectAllBlocks = function () {
  const allBlockTypes = this.blockLibStorage.getBlockTypes();
...
BlockExporterController.prototype.addBlockOptionSelectHandlers = function () {
  ...
  const updateSelectedBlockTypes_ = function (blockOption) {
    // Toggle selected.
  ...
  const makeBlockOptionSelectHandler_ = function (blockOption) {
    return function () {
  ...
  for (const blockType in this.blockOptions) {
    ...
    blockOption.dom.addEventListener(
      "click",
      makeBlockOptionSelectHandler_(blockOption)
    );
  }
};
...
BlockExporterController.prototype.selectUsedBlocks = function () {
  ...
  // factory.
  const storedBlockTypes = this.blockLibStorage.getBlockTypes();
  const sharedBlockTypes = [];
  // Keep list of custom block types used but not in library.
  const unstoredCustomBlockTypes = [];
  ...
  if (unstoredCustomBlockTypes.length > 0) {
    ...
    const blockTypesText = unstoredCustomBlockTypes.join(", ");
    const customWarning =
      "Custom blocks used in workspace factory but not " +
      "stored in block library:\n " +
      blockTypesText +
 ...
 */
BlockExporterController.prototype.setUsedBlockTypes = function (
  usedBlockTypes
) {
...
BlockExporterController.prototype.updatePreview = function () {
  ...
  const blockDefs = this.getBlockDefinitionsOfSelected();
  const genStubs = this.getGeneratorStubsOfSelected();
...
BlockExporterController.prototype.getBlockDefinitionsOfSelected = function () {
  // Get selected blocks' information.
  const blockXmlMap = this.getSelectedBlockXmlMap();
