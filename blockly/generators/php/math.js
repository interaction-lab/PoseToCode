...
Blockly.PHP.math_number = function (block) {
  ...
  const order =
    code >= 0 ? Blockly.PHP.ORDER_ATOMIC : Blockly.PHP.ORDER_UNARY_NEGATION;
  if (code == Infinity) {
    code = "INF";
...
Blockly.PHP.math_single = function (block) {
  ...
  if (operator == "NEG") {
    ...
    arg =
      Blockly.PHP.valueToCode(block, "NUM", Blockly.PHP.ORDER_UNARY_NEGATION) ||
      "0";
    if (arg[0] == "-") {
  ...
  if (operator == "SIN" || operator == "COS" || operator == "TAN") {
    arg =
      Blockly.PHP.valueToCode(block, "NUM", Blockly.PHP.ORDER_DIVISION) || "0";
  } else {
...
Blockly.PHP.math_number_property = function (block) {
  ...
  const number_to_check =
    Blockly.PHP.valueToCode(
      block,
      "NUMBER_TO_CHECK",
      Blockly.PHP.ORDER_MODULUS
    ) || "0";
  const dropdown_property = block.getFieldValue("PROPERTY");
  let code;
  if (dropdown_property == "PRIME") {
    // Prime is a special case as it is not a one-liner test.
    const functionName = Blockly.PHP.provideFunction_("math_isPrime", [
  ...
  switch (dropdown_property) {
    ...
    case "DIVISIBLE_BY":
      var divisor =
        Blockly.PHP.valueToCode(block, "DIVISOR", Blockly.PHP.ORDER_MODULUS) ||
        "0";
...
Blockly.PHP.math_change = function (block) {
  ...
  const argument0 =
    Blockly.PHP.valueToCode(block, "DELTA", Blockly.PHP.ORDER_ADDITION) || "0";
  const varName = Blockly.PHP.variableDB_.getName(
...
Blockly.PHP.math_on_list = function (block) {
  ...
  switch (func) {
    ...
    case "MAX":
      ...
      break;
    case "AVERAGE":
      var functionName = Blockly.PHP.provideFunction_("math_mean", [
        "function " + Blockly.PHP.FUNCTION_NAME_PLACEHOLDER_ + "($myList) {",
        "  return array_sum($myList) / count($myList);",
        "}",
      ]);
      list =
        Blockly.PHP.valueToCode(block, "LIST", Blockly.PHP.ORDER_NONE) ||
        "array()";
      code = functionName + "(" + list + ")";
      break;
    case "MEDIAN":
      var functionName = Blockly.PHP.provideFunction_("math_median", [
    ...
    case "MODE":
      ...
      var functionName = Blockly.PHP.provideFunction_("math_modes", [
    ...
    case "STD_DEV":
      ...
      break;
    case "RANDOM":
      var functionName = Blockly.PHP.provideFunction_("math_random_list", [
...
Blockly.PHP.math_modulo = function (block) {
  ...
  const argument0 =
    Blockly.PHP.valueToCode(block, "DIVIDEND", Blockly.PHP.ORDER_MODULUS) ||
    "0";
  const argument1 =
    Blockly.PHP.valueToCode(block, "DIVISOR", Blockly.PHP.ORDER_MODULUS) || "0";
...
Blockly.PHP.math_constrain = function (block) {
  ...
  const argument0 =
    Blockly.PHP.valueToCode(block, "VALUE", Blockly.PHP.ORDER_COMMA) || "0";
  const argument1 =
    Blockly.PHP.valueToCode(block, "LOW", Blockly.PHP.ORDER_COMMA) || "0";
  const argument2 =
    Blockly.PHP.valueToCode(block, "HIGH", Blockly.PHP.ORDER_COMMA) ||
    "Infinity";
  const code =
...
Blockly.PHP.math_random_int = function (block) {
  ...
  const argument0 =
    Blockly.PHP.valueToCode(block, "FROM", Blockly.PHP.ORDER_COMMA) || "0";
  const argument1 =
    Blockly.PHP.valueToCode(block, "TO", Blockly.PHP.ORDER_COMMA) || "0";
  const functionName = Blockly.PHP.provideFunction_("math_random_int", [
...
Blockly.PHP.math_atan2 = function (block) {
  ...
  const argument0 =
    Blockly.PHP.valueToCode(block, "X", Blockly.PHP.ORDER_COMMA) || "0";
  const argument1 =
    Blockly.PHP.valueToCode(block, "Y", Blockly.PHP.ORDER_COMMA) || "0";
  return [
