...
Blockly.PHP.controls_repeat_ext = function (block) {
  ...
  if (block.getField("TIMES")) {
    ...
    // External number.
    var repeats =
      Blockly.PHP.valueToCode(block, "TIMES", Blockly.PHP.ORDER_ASSIGNMENT) ||
      "0";
  }
  let branch = Blockly.PHP.statementToCode(block, "DO");
  branch = Blockly.PHP.addLoopTrap(branch, block);
  let code = "";
  const loopVar = Blockly.PHP.variableDB_.getDistinctName(
    "count",
...
Blockly.PHP.controls_whileUntil = function (block) {
  ...
  let argument0 =
    Blockly.PHP.valueToCode(
      block,
      "BOOL",
      until ? Blockly.PHP.ORDER_LOGICAL_NOT : Blockly.PHP.ORDER_NONE
    ) || "false";
  let branch = Blockly.PHP.statementToCode(block, "DO");
  branch = Blockly.PHP.addLoopTrap(branch, block);
  if (until) {
...
Blockly.PHP.controls_for = function (block) {
  // For loop.
  const variable0 = Blockly.PHP.variableDB_.getName(
    block.getFieldValue("VAR"),
    Blockly.VARIABLE_CATEGORY_NAME
  );
  const argument0 =
    Blockly.PHP.valueToCode(block, "FROM", Blockly.PHP.ORDER_ASSIGNMENT) || "0";
  const argument1 =
    Blockly.PHP.valueToCode(block, "TO", Blockly.PHP.ORDER_ASSIGNMENT) || "0";
  const increment =
    Blockly.PHP.valueToCode(block, "BY", Blockly.PHP.ORDER_ASSIGNMENT) || "1";
  let branch = Blockly.PHP.statementToCode(block, "DO");
  branch = Blockly.PHP.addLoopTrap(branch, block);
  let code;
  if (
    ...
    const incVar = Blockly.PHP.variableDB_.getDistinctName(
      variable0 + "_inc",
      Blockly.VARIABLE_CATEGORY_NAME
    );
    code += incVar + " = ";
...
Blockly.PHP.controls_forEach = function (block) {
  ...
  const argument0 =
    Blockly.PHP.valueToCode(block, "LIST", Blockly.PHP.ORDER_ASSIGNMENT) ||
    "[]";
  let branch = Blockly.PHP.statementToCode(block, "DO");
  branch = Blockly.PHP.addLoopTrap(branch, block);
  let code = "";
...
Blockly.PHP.controls_flow_statements = function (block) {
  ...
  if (Blockly.PHP.STATEMENT_PREFIX) {
    const loop = Blockly.Constants.Loops.CONTROL_FLOW_IN_LOOP_CHECK_MIXIN.getSurroundLoop(
      block
    );
