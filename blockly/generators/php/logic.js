...
Blockly.PHP.controls_if = function (block) {
  ...
  do {
    conditionCode =
      Blockly.PHP.valueToCode(block, "IF" + n, Blockly.PHP.ORDER_NONE) ||
      "false";
    branchCode = Blockly.PHP.statementToCode(block, "DO" + n);
    if (Blockly.PHP.STATEMENT_SUFFIX) {
      branchCode =
        Blockly.PHP.prefixLines(
...
Blockly.PHP.logic_negate = function (block) {
  ...
  const argument0 = Blockly.PHP.valueToCode(block, "BOOL", order) || "true";
...
Blockly.PHP.logic_ternary = function (block) {
  // Ternary operator.
  const value_if =
    Blockly.PHP.valueToCode(block, "IF", Blockly.PHP.ORDER_CONDITIONAL) ||
    "false";
  const value_then =
    Blockly.PHP.valueToCode(block, "THEN", Blockly.PHP.ORDER_CONDITIONAL) ||
    "null";
  const value_else =
    Blockly.PHP.valueToCode(block, "ELSE", Blockly.PHP.ORDER_CONDITIONAL) ||
    "null";
