...
Blockly.Dart.finish = function (code) {
  ...
  const imports = [];
  const definitions = [];
  for (const name in Blockly.Dart.definitions_) {
    const def = Blockly.Dart.definitions_[name];
...
Blockly.Dart.scrub_ = function (block, code, opt_thisOnly) {
  ...
  const nextCode = opt_thisOnly ? "" : Blockly.Dart.blockToCode(nextBlock);
 ...
 */
Blockly.Dart.getAdjusted = function (
  block,
  atId,
  opt_delta,
  opt_negate,
  ...
  let order = opt_order || Blockly.Dart.ORDER_NONE;
  ...
  if (delta) {
    var at =
      Blockly.Dart.valueToCode(block, atId, Blockly.Dart.ORDER_ADDITIVE) ||
      defaultAtIndex;
  } else if (opt_negate) {
    var at =
      Blockly.Dart.valueToCode(block, atId, Blockly.Dart.ORDER_UNARY_PREFIX) ||
      defaultAtIndex;
  } else {
