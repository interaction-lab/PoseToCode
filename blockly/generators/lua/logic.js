...
Blockly.Lua.controls_if = function (block) {
  ...
  do {
    conditionCode =
      Blockly.Lua.valueToCode(block, "IF" + n, Blockly.Lua.ORDER_NONE) ||
      "false";
...
Blockly.Lua.logic_compare = function (block) {
  ...
  const argument0 =
    Blockly.Lua.valueToCode(block, "A", Blockly.Lua.ORDER_RELATIONAL) || "0";
  const argument1 =
    Blockly.Lua.valueToCode(block, "B", Blockly.Lua.ORDER_RELATIONAL) || "0";
...
Blockly.Lua.logic_negate = function (block) {
  // Negation.
  const argument0 =
    Blockly.Lua.valueToCode(block, "BOOL", Blockly.Lua.ORDER_UNARY) || "true";
...
Blockly.Lua.logic_ternary = function (block) {
  // Ternary operator.
  const value_if =
    Blockly.Lua.valueToCode(block, "IF", Blockly.Lua.ORDER_AND) || "false";
  const value_then =
    Blockly.Lua.valueToCode(block, "THEN", Blockly.Lua.ORDER_AND) || "nil";
  const value_else =
    Blockly.Lua.valueToCode(block, "ELSE", Blockly.Lua.ORDER_OR) || "nil";
