...
Blockly.Lua.math_single = function (block) {
  ...
  if (operator == "POW10") {
    arg =
      Blockly.Lua.valueToCode(block, "NUM", Blockly.Lua.ORDER_EXPONENTIATION) ||
      "0";
    return ["10 ^ " + arg, Blockly.Lua.ORDER_EXPONENTIATION];
  }
  if (operator == "ROUND") {
    arg =
      Blockly.Lua.valueToCode(block, "NUM", Blockly.Lua.ORDER_ADDITIVE) || "0";
  } else {
...
Blockly.Lua.math_number_property = function (block) {
  ...
  switch (dropdown_property) {
    ...
    case "NEGATIVE":
      ...
      break;
    case "DIVISIBLE_BY":
      var divisor = Blockly.Lua.valueToCode(
        block,
        "DIVISOR",
        Blockly.Lua.ORDER_MULTIPLICATIVE
      );
...
Blockly.Lua.math_change = function (block) {
  ...
  const argument0 =
    Blockly.Lua.valueToCode(block, "DELTA", Blockly.Lua.ORDER_ADDITIVE) || "0";
...
Blockly.Lua.math_constrain = function (block) {
  ...
  const argument1 =
    Blockly.Lua.valueToCode(block, "LOW", Blockly.Lua.ORDER_NONE) ||
    "-math.huge";
  const argument2 =
    Blockly.Lua.valueToCode(block, "HIGH", Blockly.Lua.ORDER_NONE) ||
    "math.huge";
  const code =
    "math.min(math.max(" +
    argument0 +
...
Blockly.Lua.math_random_int = function (block) {
  ...
  const argument0 =
    Blockly.Lua.valueToCode(block, "FROM", Blockly.Lua.ORDER_NONE) || "0";
  const argument1 =
    Blockly.Lua.valueToCode(block, "TO", Blockly.Lua.ORDER_NONE) || "0";
...
Blockly.Lua.math_atan2 = function (block) {
  ...
  const argument0 =
    Blockly.Lua.valueToCode(block, "X", Blockly.Lua.ORDER_NONE) || "0";
  const argument1 =
    Blockly.Lua.valueToCode(block, "Y", Blockly.Lua.ORDER_NONE) || "0";
  return [
