...
Blockly.Lua.text_join = function (block) {
  ...
  if (block.itemCount_ == 0) {
    ...
    const element =
      Blockly.Lua.valueToCode(block, "ADD0", Blockly.Lua.ORDER_NONE) || "''";
...
Blockly.Lua.text_indexOf = function (block) {
  ...
  if (block.getFieldValue("END") == "FIRST") {
    var functionName = Blockly.Lua.provideFunction_("firstIndexOf", [
    ...
    var functionName = Blockly.Lua.provideFunction_("lastIndexOf", [
      "function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(str, substr)",
...
Blockly.Lua.text_charAt = function (block) {
  ...
  const atOrder =
    where == "FROM_END" ? Blockly.Lua.ORDER_UNARY : Blockly.Lua.ORDER_NONE;
  const at = Blockly.Lua.valueToCode(block, "AT", atOrder) || "1";
  const text =
    Blockly.Lua.valueToCode(block, "VALUE", Blockly.Lua.ORDER_NONE) || "''";
  let code;
  if (where == "RANDOM") {
    var functionName = Blockly.Lua.provideFunction_("text_random_letter", [
    ...
    if (start.match(/^-?\w*$/)) {
      ...
      var functionName = Blockly.Lua.provideFunction_("text_char_at", [
...
Blockly.Lua.text_getSubstring = function (block) {
  // Get substring.
  const text =
    Blockly.Lua.valueToCode(block, "STRING", Blockly.Lua.ORDER_NONE) || "''";

  // Get start index.
  const where1 = block.getFieldValue("WHERE1");
  const at1Order =
    where1 == "FROM_END" ? Blockly.Lua.ORDER_UNARY : Blockly.Lua.ORDER_NONE;
  ...
  const at2Order =
    where2 == "FROM_END" ? Blockly.Lua.ORDER_UNARY : Blockly.Lua.ORDER_NONE;
...
Blockly.Lua.text_count = function (block) {
  const text =
    Blockly.Lua.valueToCode(block, "TEXT", Blockly.Lua.ORDER_NONE) || "''";
  const sub =
    Blockly.Lua.valueToCode(block, "SUB", Blockly.Lua.ORDER_NONE) || "''";
...
Blockly.Lua.text_replace = function (block) {
  const text =
    Blockly.Lua.valueToCode(block, "TEXT", Blockly.Lua.ORDER_NONE) || "''";
  const from =
    Blockly.Lua.valueToCode(block, "FROM", Blockly.Lua.ORDER_NONE) || "''";
  const to =
    Blockly.Lua.valueToCode(block, "TO", Blockly.Lua.ORDER_NONE) || "''";
...
Blockly.Lua.text_reverse = function (block) {
  const text =
    Blockly.Lua.valueToCode(block, "TEXT", Blockly.Lua.ORDER_HIGH) || "''";
