...
Blockly.Lua.lists_create_with = function (block) {
  ...
  for (let i = 0; i < block.itemCount_; i++) {
    elements[i] =
      Blockly.Lua.valueToCode(block, "ADD" + i, Blockly.Lua.ORDER_NONE) ||
      "None";
  }
...
Blockly.Lua.lists_repeat = function (block) {
  ...
  const element =
    Blockly.Lua.valueToCode(block, "ITEM", Blockly.Lua.ORDER_NONE) || "None";
  const repeatCount =
    Blockly.Lua.valueToCode(block, "NUM", Blockly.Lua.ORDER_NONE) || "0";
...
Blockly.Lua.lists_length = function (block) {
  ...
  const list =
    Blockly.Lua.valueToCode(block, "VALUE", Blockly.Lua.ORDER_UNARY) || "{}";
...
Blockly.Lua.lists_indexOf = function (block) {
  ...
  const item =
    Blockly.Lua.valueToCode(block, "FIND", Blockly.Lua.ORDER_NONE) || "''";
  const list =
    Blockly.Lua.valueToCode(block, "VALUE", Blockly.Lua.ORDER_NONE) || "{}";
  if (block.getFieldValue("END") == "FIRST") {
    var functionName = Blockly.Lua.provideFunction_("first_index", [
    ...
    var functionName = Blockly.Lua.provideFunction_("last_index", [
      "function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(t, elem)",
      "  for i = #t, 1, -1 do",
      "    if t[i] == elem then",
      "      return i",
...
Blockly.Lua.lists_getIndex = function (block) {
  ...
  const list =
    Blockly.Lua.valueToCode(block, "VALUE", Blockly.Lua.ORDER_HIGH) || "({})";
  ...
  if (
    ...
    if (mode == "REMOVE") {
      ...
      const listVar = Blockly.Lua.variableDB_.getDistinctName(
        "tmp_list",
      ...
      var at =
        Blockly.Lua.valueToCode(block, "AT", Blockly.Lua.ORDER_NONE) || "1";
      if (mode == "GET") {
...
Blockly.Lua.lists_setIndex = function (block) {
  ...
  let list =
    Blockly.Lua.valueToCode(block, "LIST", Blockly.Lua.ORDER_HIGH) || "{}";
  const mode = block.getFieldValue("MODE") || "SET";
  const where = block.getFieldValue("WHERE") || "FROM_START";
  const at =
    Blockly.Lua.valueToCode(block, "AT", Blockly.Lua.ORDER_ADDITIVE) || "1";
  const value =
    Blockly.Lua.valueToCode(block, "TO", Blockly.Lua.ORDER_NONE) || "None";
  const getIndex_ = Blockly.Lua.lists.getIndex_;
...
Blockly.Lua.lists_getSublist = function (block) {
  // Get sublist.
  const list =
    Blockly.Lua.valueToCode(block, "LIST", Blockly.Lua.ORDER_NONE) || "{}";
  const where1 = block.getFieldValue("WHERE1");
  const where2 = block.getFieldValue("WHERE2");
  const at1 =
    Blockly.Lua.valueToCode(block, "AT1", Blockly.Lua.ORDER_NONE) || "1";
  const at2 =
    Blockly.Lua.valueToCode(block, "AT2", Blockly.Lua.ORDER_NONE) || "1";
  const getIndex_ = Blockly.Lua.lists.getIndex_;
...
Blockly.Lua.lists_split = function (block) {
  ...
  const delimiter =
    Blockly.Lua.valueToCode(block, "DELIM", Blockly.Lua.ORDER_NONE) || "''";
