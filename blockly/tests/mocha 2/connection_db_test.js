...
suite("Connection Database", function () {
    ...
    this.assertOrder = function () {
      ...
      for (let i = 1; i < length; i++) {
        chai.assert.isAtMost(
          this.database.connections_[i - 1].y,
  ...
  test("Add Connection", function () {
    ...
    chai.assert.sameOrderedMembers(this.database.connections_, [y2]);

    this.database.addConnection(y4, 4);
    chai.assert.sameOrderedMembers(this.database.connections_, [y2, y4]);

    this.database.addConnection(y1, 1);
    chai.assert.sameOrderedMembers(this.database.connections_, [y1, y2, y4]);

    this.database.addConnection(y3a, 3);
    chai.assert.sameOrderedMembers(this.database.connections_, [
      y1,
      y2,
      y3a,
      y4,
  ...
  test("Remove Connection", function () {
    ...
    chai.assert.sameOrderedMembers(this.database.connections_, [y3a, y3b, y3c]);

    this.database.removeConnection(y3a, 3);
    chai.assert.sameOrderedMembers(this.database.connections_, [y3b, y3c]);

    this.database.removeConnection(y3c, 3);
    chai.assert.sameOrderedMembers(this.database.connections_, [y3b]);
  ...
  suite("Get Neighbors", function () {
    ...
    test("Block At Top", function () {
      ...
      chai.assert.sameMembers(
        neighbors,
        this.database.connections_.slice(0, 5)
      );
    });
    test("Block In Middle", function () {
      ...
      chai.assert.sameMembers(
        neighbors,
        this.database.connections_.slice(2, 7)
      );
    });
    test("Block At End", function () {
      ...
      chai.assert.sameMembers(
        neighbors,
        this.database.connections_.slice(5, 10)
      );
    });
  ...
  suite("Search For Closest", function () {
    setup(function () {
      // Ignore type checks.
      sinon
        .stub(this.database.connectionChecker_, "doTypeChecks")
        .returns(true);
      // Ignore safety checks.
      sinon
        .stub(this.database.connectionChecker_, "doSafetyChecks")
        .returns(Blockly.Connection.CAN_CONNECT);
      // Skip everything but the distance checks.
      sinon
        .stub(this.database.connectionChecker_, "doDragChecks")
