...
Blockly.Lua.unittest_main = function (block) {
  ...
  const resultsVar = Blockly.Lua.variableDB_.getName('unittestResults',
...
Blockly.Lua.unittest_assertequals = function (block) {
  // Asserts that a value equals another value.
  const message = Blockly.Lua.valueToCode(block, 'MESSAGE',
    Blockly.Lua.ORDER_NONE) || ''
  const actual = Blockly.Lua.valueToCode(block, 'ACTUAL',
    Blockly.Lua.ORDER_NONE) || 'nil'
  const expected = Blockly.Lua.valueToCode(block, 'EXPECTED',
    Blockly.Lua.ORDER_NONE) || 'nil'
...
Blockly.Lua.unittest_adjustindex = function (block) {
  const index = Blockly.Lua.valueToCode(block, 'INDEX',
