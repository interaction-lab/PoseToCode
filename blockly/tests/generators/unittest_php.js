...
Blockly.PHP.unittest_main = function (block) {
  ...
  const resultsVar = Blockly.PHP.variableDB_.getName('unittestResults',
...
Blockly.PHP.unittest_assertequals = function (block) {
  // Asserts that a value equals another value.
  const message = Blockly.PHP.valueToCode(block, 'MESSAGE',
    Blockly.PHP.ORDER_NONE) || ''
  const actual = Blockly.PHP.valueToCode(block, 'ACTUAL',
    Blockly.PHP.ORDER_COMMA) || 'null'
  const expected = Blockly.PHP.valueToCode(block, 'EXPECTED',
    Blockly.PHP.ORDER_COMMA) || 'null'
...
Blockly.PHP.unittest_adjustindex = function (block) {
  let index = Blockly.PHP.valueToCode(block, 'INDEX',
