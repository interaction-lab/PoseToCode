...
function test_count_by() {
  ...
  loglist = [];
  const x_start3 = 5 + 0.5;
  const x_end3 = 1 + 0;
  let x_inc3 = Math.abs(1 + 0);
  if (x_start3 > x_end3) {
    x_inc3 = -x_inc3;
  }
...
function test_continue() {
  ...
  log = "";
  const x_list2 = ["a", "b", "c", "d"];
  for (const x_index2 in x_list2) {
    x = x_list2[x_index2];
    if (x == "c") {
      continue;
    }
...
function test_break() {
  ...
  log = "";
  const x_list3 = ["a", "b", "c", "d"];
  for (const x_index3 in x_list3) {
    x = x_list3[x_index3];
    if (x == "c") {
      break;
    }
...
function test_operations_on_list() {
  ...
  assertEquals(
    [3, 4, 5].indexOf(mathRandomList([3, 4, 5])) + 1 > 0,
    true,
    "random"
  );
}
...
function get_fruit() {
  number_of_calls =
    (typeof number_of_calls === "number" ? number_of_calls : 0) + 1;
  return "Banana";
}
...
function test_find_text_complex() {
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_fruit() : null).indexOf("an") + 1,
    2,
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_fruit() : null).lastIndexOf("an") + 1,
    4,
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_fruit() : null).indexOf("Peel") + 1,
    0,
...
function test_get_text_simple() {
  ...
  assertEquals(
    text.indexOf(textRandomLetter(text)) + 1 > 0,
    true,
    "get random simple"
  );
...
function test_get_text_complex() {
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_Blockly() : null).charAt(0),
    "B",
  ...
  number_of_calls = 0;
  assertEquals(
    text.indexOf(textRandomLetter(get_Blockly())) + 1 > 0,
    true,
    "get random complex"
  );
  ...
  number_of_calls = 0;
  assertEquals(
    text.indexOf(textRandomLetter(true ? get_Blockly() : null)) + 1 > 0,
    true,
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_Blockly() : null).charAt((true ? 3 : null) - 1),
    "o",
    "get # order complex"
  );
  ...
  // The order for index for #-end is addition because this will catch errors in generators where most perform the operation ... - index.
  assertEquals(
    (true ? get_Blockly() : null).slice(-(0 + 3)).charAt(0),
    "k",
...
function test_substring_simple() {
  ...
  assertEquals(
    text.slice((true ? 2 : null) - 1, true ? 3 : null),
    "23",
    "substring # simple order"
  );
  assertEquals(
    text.slice(text.length - 3, text.length - 1),
    "78",
    "substring #-end simple"
  );
  // The order for index for #-end is addition because this will catch errors in generators where most perform the operation ... - index.
  assertEquals(
    text.slice(text.length - (0 + 3), text.length - (0 + 2 - 1)),
    "78",
  ...
  assertEquals(
    text.slice(0, text.length - 0),
    "123456789",
    "substring all with # #-end simple"
  );
  assertEquals(
    text.slice(text.length - 9, 9),
    "123456789",
    "substring all with #-end # simple"
  );
  ...
  assertEquals(
    text.slice(0 + 1 - 1, text.length - (0 + 1 - 1)),
    "123456789",
...
function subsequenceFromEndFromEnd(sequence, at1, at2) {
  const start = sequence.length - 1 - at1;
  const end = sequence.length - 1 - at2 + 1;
...
function test_substring_complex() {
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_numbers() : null).slice((true ? 2 : null) - 1, true ? 3 : null),
    "23",
  ...
  // The order for index for #-end is addition because this will catch errors in generators where most perform the operation ... - index.
  assertEquals(
    subsequenceFromEndFromEnd(get_numbers(), 2, 1),
    "78",
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromStartFromEnd(get_numbers(), 1, 1),
    "2345678",
    "substring # #-end complex"
  );
  check_number_of_calls("substring # #-end complex");
  number_of_calls = 0;
  assertEquals(
    subsequenceFromEndFromStart(get_numbers(), 6, 3),
    "34",
    "substring #-end # complex"
  );
  check_number_of_calls("substring #-end # complex");
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFirstFromEnd(get_numbers(), 1),
    "12345678",
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromStartLast(get_numbers(), 6),
    "789",
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromEndLast(get_numbers(), 2),
    "789",
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromStartFromEnd(get_numbers(), 0, 0),
    "123456789",
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromEndFromStart(get_numbers(), 8, 8),
    "123456789",
    "substring all with #-end # complex"
  );
  check_number_of_calls("substring all with #-end # complex");
  ...
  // Checks that the whole string is properly retrieved even if the value for start and end is not a simple number. This is especially important in generators where substring uses [x:length - y] for # #-end.
  assertEquals(
    subsequenceFromStartFromEnd(get_numbers(), 0 + 1 - 1, 0 + 1 - 1),
    "123456789",
...
function test_case() {
  ...
  assertEquals(
    textToTitleCase(true ? text : null),
    "Hello World",
    "titlecase order"
  );
}
...
function test_text_reverse() {
  ...
  assertEquals(
    "woolloomooloo".split("").reverse().join(""),
    "ooloomoolloow",
    "longer"
  );
}
...
function test_replace() {
  assertEquals(
    textReplace("woolloomooloo", "oo", "123"),
    "w123ll123m123l123",
    "replace all instances 1"
  );
  assertEquals(
    textReplace("woolloomooloo", ".oo", "X"),
    "woolloomooloo",
    "literal string replacement"
  );
  assertEquals(
    textReplace("woolloomooloo", "abc", "X"),
    "woolloomooloo",
    "not found"
  );
  assertEquals(
    textReplace("woolloomooloo", "o", ""),
    "wllml",
    "empty replacement 1"
  );
...
function test_find_lists_complex() {
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_names() : null).indexOf("Eve") + 1,
    2,
    "find first order complex"
  );
  check_number_of_calls("find first order complex");
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_names() : null).lastIndexOf("Eve") + 1,
    4,
    "find last order complex"
  );
  check_number_of_calls("find last order complex");
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_names() : null).indexOf("Dave") + 1,
    0,
    "find none order complex"
  );
  check_number_of_calls("find none order complex");
...
function test_get_lists_simple() {
  ...
  assertEquals(
    list.indexOf(listsGetRandomItem(list, false)) + 1 > 0,
    true,
    "get random simple"
  );
...
function test_get_lists_create_list() {
  ...
  assertEquals(
    ["Kirk", "Spock", "McCoy"].slice(-1)[0],
    "McCoy",
    "get last simple"
  );
  assertEquals(
    ["Kirk", "Spock", "McCoy"].indexOf(
      listsGetRandomItem(["Kirk", "Spock", "McCoy"], false)
    ) +
      1 >
      0,
    true,
  ...
  assertEquals(
    ["Kirk", "Spock", "McCoy"][(true ? 2 : null) - 1],
    "Spock",
    "get # order simple"
  );
  assertEquals(
    ["Kirk", "Spock", "McCoy"].slice(-3)[0],
    "Kirk",
    "get #-end simple"
  );
  // The order for index for #-end is addition because this will catch errors in generators where most perform the operation ... - index.
  assertEquals(
    ["Kirk", "Spock", "McCoy"].slice(-(0 + 3))[0],
    "Kirk",
...
function test_get_lists_complex() {
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_star_wars() : null).slice(-1)[0],
    "McCoy",
    "get last order complex"
  );
  check_number_of_calls("get last order complex");
  number_of_calls = 0;
  assertEquals(
    list.indexOf(listsGetRandomItem(get_star_wars(), false)) + 1 > 0,
    true,
    "get random complex"
  );
  check_number_of_calls("get random complex");
  number_of_calls = 0;
  assertEquals(
    list.indexOf(listsGetRandomItem(true ? get_star_wars() : null, false)) + 1 >
      0,
    true,
    "get random order complex"
  );
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_star_wars() : null)[(true ? 2 : null) - 1],
    "Spock",
    "get # order complex"
  );
  check_number_of_calls("get # order complex");
  ...
  // The order for index for #-end is addition because this will catch errors in generators where most perform the operation ... - index.
  assertEquals(
    (true ? get_star_wars() : null).slice(-(0 + 3))[0],
    "Kirk",
    "get #-end order complex"
  );
  check_number_of_calls("get #-end order complex");
...
function test_getRemove() {
  ...
  list = ["Kirk", "Spock", "McCoy"];
  assertEquals(
    list.indexOf(listsGetRandomItem(list, true)) + 1 == 0,
    true,
    "getremove random"
  );
  ...
  list = ["Kirk", "Spock", "McCoy"];
  assertEquals(
    list.indexOf(listsGetRandomItem(true ? list : null, true)) + 1 == 0,
    true,
  ...
  list = ["Kirk", "Spock", "McCoy"];
  assertEquals(
    (true ? list : null).splice((true ? 2 : null) - 1, 1)[0],
    "Spock",
    "getremove # order"
  );
...
function test_set() {
  ...
  list = ["Picard", "Riker", "Crusher"];
  const tmpList = true ? list : null;
  tmpList[tmpList.length - 1] = "Beverly";
  assertEquals(list, ["Picard", "Riker", "Beverly"], "set last order list");
  list = ["Picard", "Riker", "Crusher"];
  const tmpX = Math.floor(Math.random() * list.length);
  list[tmpX] = "Data";
  assertEquals(list.length, 3, "set random list");
  list = ["Picard", "Riker", "Crusher"];
  const tmpList2 = true ? list : null;
  const tmpX2 = Math.floor(Math.random() * tmpList2.length);
  tmpList2[tmpX2] = "Data";
...
function test_insert() {
  ...
  list = ["Picard", "Riker", "Crusher"];
  const tmpX3 = Math.floor(Math.random() * list.length);
  list.splice(tmpX3, 0, "Data");
  assertEquals(list.length, 4, "insert random list");
  list = ["Picard", "Riker", "Crusher"];
  const tmpList4 = true ? list : null;
  const tmpX4 = Math.floor(Math.random() * tmpList4.length);
  ...
  // The order for index for #-end is addition because this will catch
  // errors in generators where most perform the operation ... - index.
  const tmpList5 = true ? list : null;
  tmpList5.splice(tmpList5.length - (0 + 2), 0, "Data");
  assertEquals(
    list,
    ["Picard", "Data", "Riker", "Crusher"],
...
function test_sublist_simple() {
  ...
  assertEquals(
    list.slice((true ? 2 : null) - 1, true ? 3 : null),
    ["Challenger", "Discovery"],
    "sublist # simple order"
  );
  assertEquals(
    list.slice(list.length - 3, list.length - 1),
    ["Discovery", "Atlantis"],
    "sublist #-end simple"
  );
  // The order for index for #-end is addition because this will catch errors in generators where most perform the operation ... - index.
  assertEquals(
    list.slice(list.length - (0 + 3), list.length - (0 + 2 - 1)),
    ["Discovery", "Atlantis"],
  ...
  assertEquals(
    list.slice(list.length - 4, list.length),
    ["Challenger", "Discovery", "Atlantis", "Endeavour"],
    "sublist #-end last simple"
  );
  assertEquals(
    list.slice(0, list.length - 0),
    list,
    "sublist all with # #-end simple"
  );
  assertEquals(
    list.slice(list.length - 5, 5),
    list,
    "sublist all with #-end # simple"
  );
  ...
  assertEquals(
    list.slice(0 + 1 - 1, list.length - (0 + 1 - 1)),
    list,
...
function test_sublist_complex() {
  ...
  number_of_calls = 0;
  assertEquals(
    (true ? get_space_shuttles() : null).slice(
      (true ? 2 : null) - 1,
      true ? 3 : null
    ),
    ["Challenger", "Discovery"],
  ...
  // The order for index for #-end is addition because this will catch errors in generators where most perform the operation ... - index.
  assertEquals(
    subsequenceFromEndFromEnd(get_space_shuttles(), 2, 1),
    ["Discovery", "Atlantis"],
    "sublist # end complex"
  );
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromStartFromEnd(get_space_shuttles(), 1, 1),
    ["Challenger", "Discovery", "Atlantis"],
    "sublist # #-end complex"
  );
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromEndFromStart(get_space_shuttles(), 2, 3),
    ["Discovery", "Atlantis"],
    "sublist #-end # complex"
  );
  check_number_of_calls("sublist #-end # complex");
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFirstFromEnd(get_space_shuttles(), 3),
    ["Columbia", "Challenger"],
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromStartLast(get_space_shuttles(), 3),
    ["Atlantis", "Endeavour"],
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromEndLast(get_space_shuttles(), 3),
    ["Challenger", "Discovery", "Atlantis", "Endeavour"],
    "sublist #-end last simple"
  );
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromStartFromEnd(get_space_shuttles(), 0, 0),
    list,
  ...
  number_of_calls = 0;
  assertEquals(
    subsequenceFromEndFromStart(get_space_shuttles(), 4, 4),
    list,
    "sublist all with #-end # complex"
  );
  check_number_of_calls("sublist all with #-end # complex");
  ...
  // Checks that the whole list is properly retrieved even if the value for start and end is not a simple number. This is especially important in generators where sublist uses [x:length - y] for # #-end.
  assertEquals(
    subsequenceFromStartFromEnd(get_space_shuttles(), 0 + 1 - 1, 0 + 1 - 1),
    list,
...
function test_sort_alphabetic() {
  list = ["Vulcan", "klingon", "Borg"];
  assertEquals(
    list.slice().sort(listsGetSortCompare("TEXT", 1)),
    ["Borg", "Vulcan", "klingon"],
    "sort alphabetic ascending"
  );
  assertEquals(
    (true ? list : null).slice().sort(listsGetSortCompare("TEXT", 1)),
    ["Borg", "Vulcan", "klingon"],
...
function test_sort_ignoreCase() {
  list = ["Vulcan", "klingon", "Borg"];
  assertEquals(
    list.slice().sort(listsGetSortCompare("IGNORE_CASE", 1)),
    ["Borg", "klingon", "Vulcan"],
    "sort ignore case ascending"
  );
  assertEquals(
    (true ? list : null).slice().sort(listsGetSortCompare("IGNORE_CASE", 1)),
    ["Borg", "klingon", "Vulcan"],
...
function test_sort_numeric() {
  list = [8, 18, -1];
  assertEquals(
    list.slice().sort(listsGetSortCompare("NUMERIC", -1)),
    [18, 8, -1],
    "sort numeric descending"
  );
  assertEquals(
    (true ? list : null).slice().sort(listsGetSortCompare("NUMERIC", -1)),
    [18, 8, -1],
...
function test_colour_random() {
  for (let count4 = 0; count4 < 100; count4++) {
    ...
    assertEquals(
      item.length,
      7,
      "length of random colour string: " + String(item)
    );
    assertEquals(
      item.charAt(0),
      "#",
      "format of random colour string: " + String(item)
    );
    for (i = 1; i <= 6; i++) {
      assertEquals(
        "abcdefABDEF0123456789".indexOf(item.charAt(i + 1 - 1)) + 1 != 0,
        true,
        ["contents of random colour string: ", item, " at index: ", i + 1].join(
          ""
        )
      );
    }
...
function test_blend() {
  assertEquals(
    colourBlend("#ff0000", colourRgb(100, 40, 0), 0.4),
    "#ff2900",
    "blend"
  );
}
