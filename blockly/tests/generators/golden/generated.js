...
let unittestResults, test_name, naked, proc_x, proc_y, func_x, func_y, func_a, n, ok, log, count, varToChange, rand, item, text, number_of_calls, list, proc_z, func_z, x, proc_w, func_c, if2, i, loglist, changing_list, list_copy
...
function mathStandardDeviation (numbers) {
  ...
  if (!n) return null
  const mean = numbers.reduce(function (x, y) { return x + y }) / n
  let variance = 0
  for (let j = 0; j < n; j++) {
...
function test_colour_random () {
  for (let count4 = 0; count4 < 100; count4++) {
    ...
    for (i = 1; i <= 6; i++) {
      assertEquals('abcdefABDEF0123456789'.indexOf(item.charAt(((i + 1) - 1))) + 1 != 0, true, ['contents of random colour string: ', item, ' at index: ', i + 1].join(''))
    }
  }
}
