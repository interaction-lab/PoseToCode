...
suite('ASTNode', function () {
  ...
  suite('HelperFunctions', function () {
    test('findNextForInput_', function () {
      ...
      const node = Blockly.ASTNode.createConnectionNode(connection)
    ...
    test('findNextForField_', function () {
      ...
      const node = Blockly.ASTNode.createFieldNode(field)
    ...
    test('findPrevForField_', function () {
      const field = this.blocks.statementInput1.inputList[0].fieldRow[0]
      const field2 = this.blocks.statementInput1.inputList[0].fieldRow[1]
      const node = Blockly.ASTNode.createFieldNode(field2)
    ...
    test('navigateBetweenStacks_Forward', function () {
      const node = new Blockly.ASTNode(
  ...
  suite('NavigationFunctions', function () {
    ...
    suite('Next', function () {
      ...
      test('fromPreviousToBlock', function () {
        ...
        const node = Blockly.ASTNode.createConnectionNode(prevConnection)
      ...
      test('fromBlockToNext', function () {
        ...
        const node = Blockly.ASTNode.createBlockNode(this.blocks.statementInput1)
        const nextNode = node.next()
        chai.assert.equal(nextNode.getLocation(), nextConnection)
      })
      test('fromBlockToNull', function () {
        const node = Blockly.ASTNode.createBlockNode(this.blocks.noNextConnection)
        const nextNode = node.next()
      ...
      test('fromNextToPrevious', function () {
        ...
        const node = Blockly.ASTNode.createConnectionNode(nextConnection)
        const nextNode = node.next()
      ...
      test('fromInputToInput', function () {
        ...
        const node = Blockly.ASTNode.createInputNode(input)
        const nextNode = node.next()
      ...
      test('fromOutputToBlock', function () {
        ...
        const node = Blockly.ASTNode.createConnectionNode(output)
        const nextNode = node.next()
      ...
      test('fromStackToStack', function () {
        const node = Blockly.ASTNode.createStackNode(this.blocks.statementInput1)
        const nextNode = node.next()
      ...
      test('fromStackToNull', function () {
        const node = Blockly.ASTNode.createStackNode(this.blocks.singleBlock)
        const nextNode = node.next()
        chai.assert.isNull(nextNode)
      })
    })
    ...
    suite('Previous', function () {
      ...
      test('fromBlockToNull', function () {
        const node = Blockly.ASTNode.createBlockNode(this.blocks.noPrevConnection)
        const prevNode = node.prev()
        chai.assert.isNull(prevNode)
      })
      test('fromBlockToOutput', function () {
        const node = Blockly.ASTNode.createBlockNode(this.blocks.fieldWithOutput)
        const prevNode = node.prev()
      ...
      test('fromStackToStack', function () {
        const node = Blockly.ASTNode.createStackNode(this.blocks.statementInput4)
        const prevNode = node.prev()
        chai.assert.equal(prevNode.getLocation(), this.blocks.statementInput1)
    ...
    suite('In', function () {
      ...
      test('fromBlockToInput', function () {
        ...
        const node = Blockly.ASTNode.createBlockNode(this.blocks.valueInput)
        const inNode = node.in()
      ...
      test('fromBlockToNull_DummyInput', function () {
        const node = Blockly.ASTNode.createBlockNode(this.blocks.dummyInput)
        const inNode = node.in()
        chai.assert.isNull(inNode)
      })
      test('fromBlockToInput_DummyInputValue', function () {
        const node = Blockly.ASTNode.createBlockNode(this.blocks.dummyInputValue)
      ...
      test('fromWorkspaceToStack', function () {
        ...
        const node = Blockly.ASTNode.createWorkspaceNode(this.workspace, coordinate)
        const inNode = node.in()
      ...
      test('fromStackToOutput', function () {
        const node = Blockly.ASTNode.createStackNode(this.blocks.fieldWithOutput2)
      ...
      test('fromStackToBlock', function () {
        const node = Blockly.ASTNode.createStackNode(this.blocks.dummyInput)
        const inNode = node.in()
    ...
    suite('Out', function () {
      ...
      test('fromPreviousToInput', function () {
        ...
        const node = Blockly.ASTNode.createConnectionNode(previous)
        const outNode = node.out()
        chai.assert.equal(outNode.getType(), Blockly.ASTNode.types.INPUT)
      ...
      test('fromNextToInput', function () {
        ...
        const inputConnection = this.blocks.statementInput2.inputList[1].connection
        const node = Blockly.ASTNode.createConnectionNode(next)
        const outNode = node.out()
        chai.assert.equal(outNode.getType(), Blockly.ASTNode.types.INPUT)
        chai.assert.equal(outNode.getLocation(), inputConnection)
      })
      ...
      test('fromBlockToStack', function () {
        const node = Blockly.ASTNode.createBlockNode(this.blocks.statementInput2)
        const outNode = node.out()
        chai.assert.equal(outNode.getType(), Blockly.ASTNode.types.STACK)
        chai.assert.equal(outNode.getLocation(), this.blocks.statementInput1)
      })
      test('fromBlockToInput', function () {
        ...
        const node = Blockly.ASTNode.createBlockNode(this.blocks.statementInput3)
        const outNode = node.out()
        chai.assert.equal(outNode.getType(), Blockly.ASTNode.types.INPUT)
      ...
      test('fromBlockToStack_OutputConnection', function () {
        const node = Blockly.ASTNode.createBlockNode(this.blocks.fieldWithOutput2)
        const outNode = node.out()
        chai.assert.equal(outNode.getType(), Blockly.ASTNode.types.STACK)
        chai.assert.equal(outNode.getLocation(), this.blocks.fieldWithOutput2)
      })
      test('fromBlockToInput_OutputConnection', function () {
        const node = Blockly.ASTNode.createBlockNode(this.blocks.outputNextBlock)
    ...
    suite('createFunctions', function () {
      ...
      test('createTopNode-previous', function () {
        ...
        const topNode = Blockly.ASTNode.createTopNode(block)
