...
suite('Cursor', function () {
  ...
  test('Next - From a Previous skip over next connection and block', function () {
    const prevNode = Blockly.ASTNode.createConnectionNode(this.blocks.A.previousConnection)
  ...
  test('Next - From last block in a stack go to next connection', function () {
    const prevNode = Blockly.ASTNode.createConnectionNode(this.blocks.B.previousConnection)
    this.cursor.setCurNode(prevNode)
    this.cursor.next()
    const curNode = this.cursor.getCurNode()
  ...
  test('In - From output connection', function () {
    ...
    const outputNode = Blockly.ASTNode.createConnectionNode(fieldBlock.outputConnection)
  ...
  test('Prev - From previous connection skip over next connection', function () {
    ...
    const prevConnectionNode = Blockly.ASTNode.createConnectionNode(prevConnection)
