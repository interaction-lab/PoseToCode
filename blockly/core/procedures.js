...
Blockly.Procedures.mutatorOpenListener = function (e) {
  if (
    ...
    e.element != "mutatorOpen" ||
    !e.newValue
  ) {
    return;
  }
  const workspaceId = /** @type {string} */ (e.workspaceId);
  const block = Blockly.Workspace.getById(workspaceId).getBlockById(e.blockId);
...
Blockly.Procedures.mutateCallers = function (defBlock) {
  ...
  const procedureBlock = /** @type {!Blockly.Procedures.ProcedureBlock} */ (defBlock);
