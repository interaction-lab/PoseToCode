...
Blockly.ContextMenuItems.addDeletableBlocks_ = function (block, deleteList) {
  if (block.isDeletable()) {
    ...
    const children = /** @type !Array.<!Blockly.BlockSvg> */ (block.getChildren(
      false
    ));
...
Blockly.ContextMenuItems.registerDeleteAll = function () {
  ...
  const deleteOption = {
    displayText: function (
      ...
      const deletableBlocksLength = Blockly.ContextMenuItems.getDeletableBlocks_(
        scope.workspace
      ).length;
    ...
    preconditionFn: function (
      ...
      if (!block.isInFlyout && block.isMovable() && !block.isCollapsed()) {
        for (let i = 1; i < block.inputList.length; i++) {
          ...
          if (
            block.inputList[i - 1].type != Blockly.NEXT_STATEMENT &&
