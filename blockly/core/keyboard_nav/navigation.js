...
Blockly.navigation.modifyWarn_ = function () {
  ...
  const cursorNode = Blockly.navigation.getNavigationWorkspace()
...
Blockly.navigation.connect_ = function (movingConnection, destConnection) {
  ...
  const movingInferior = Blockly.navigation.getInferiorConnection_(movingConnection)
  const destSuperior = Blockly.navigation.getSuperiorConnection_(destConnection)

  const movingSuperior = Blockly.navigation.getSuperiorConnection_(movingConnection)
  const destInferior = Blockly.navigation.getInferiorConnection_(destConnection)
...
Blockly.navigation.disconnectBlocks_ = function () {
  ...
  const connectionNode = Blockly.ASTNode.createConnectionNode(superiorConnection)
...
Blockly.navigation.toolboxOnAction_ = function (action) {
  const workspace = Blockly.navigation.getNavigationWorkspace()
  const toolbox = workspace.getToolbox()
  const handled = toolbox && typeof toolbox.onBlocklyAction === 'function'
