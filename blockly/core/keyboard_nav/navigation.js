...
Blockly.navigation.focusToolbox_ = function () {
  const toolbox = Blockly.navigation.getNavigationWorkspace().getToolbox();
  if (toolbox) {
...
Blockly.navigation.focusFlyout_ = function () {
  ...
  const workspace = Blockly.navigation.getNavigationWorkspace();
...
Blockly.navigation.insertFromFlyout = function () {
  ...
  const curBlock = /** @type {!Blockly.BlockSvg} */ (Blockly.navigation
    .getFlyoutCursor_()
    .getCurNode()
    .getLocation());
...
Blockly.navigation.modifyWarn_ = function () {
  const markerNode = Blockly.navigation.getMarker().getCurNode();
  const cursorNode = Blockly.navigation
    .getNavigationWorkspace()
    .getCursor()
    .getCurNode();
...
Blockly.navigation.modify_ = function () {
  ...
  let cursorLoc = cursorNode.getLocation();
  let markerLoc = markerNode.getLocation();
...
Blockly.navigation.connect_ = function (movingConnection, destConnection) {
  ...
  const movingInferior = Blockly.navigation.getInferiorConnection_(
    movingConnection
  );
  const destSuperior = Blockly.navigation.getSuperiorConnection_(
    destConnection
  );

  const movingSuperior = Blockly.navigation.getSuperiorConnection_(
    movingConnection
  );
  const destInferior = Blockly.navigation.getInferiorConnection_(
    destConnection
  );
  ...
  if (
    ...
    Blockly.navigation.moveAndConnect_(movingConnection, destConnection)
  ) {
    return true;
  } else {
    const checker = movingConnection.getConnectionChecker();
...
Blockly.navigation.insertBlock = function (block, destConnection) {
  switch (destConnection.type) {
    ...
    case Blockly.NEXT_STATEMENT:
      if (
        Blockly.navigation.connect_(block.previousConnection, destConnection)
      ) {
        return true;
      }
      break;
    ...
    case Blockly.OUTPUT_VALUE:
      for (let i = 0; i < block.inputList.length; i++) {
        const inputConnection = /** @type {Blockly.RenderedConnection} */ (block
          .inputList[i].connection);
        if (
          inputConnection &&
...
Blockly.navigation.disconnectBlocks_ = function () {
  ...
  const curConnection = /** @type {!Blockly.RenderedConnection} */ (curNode.getLocation());
  ...
  const connectionNode = Blockly.ASTNode.createConnectionNode(
    superiorConnection
  );
...
Blockly.navigation.removeMark_ = function () {
  const marker = Blockly.navigation.getMarker();
...
Blockly.navigation.moveCursorOnBlockMutation = function (mutatedBlock) {
  const cursor = Blockly.navigation.getNavigationWorkspace().getCursor();
  if (cursor) {
    const curNode = cursor.getCurNode();
    const block = curNode ? curNode.getSourceBlock() : null;
...
Blockly.navigation.onKeyPress = function (e) {
  const key = Blockly.user.keyMap.serializeKeyEvent(e);
  const action = Blockly.user.keyMap.getActionByKeyCode(key);
...
Blockly.navigation.handleActions_ = function (action) {
  if (
    action.name == Blockly.navigation.actionNames.TOOLBOX ||
    Blockly.navigation.currentState_ == Blockly.navigation.STATE_TOOLBOX
  ) {
...
Blockly.navigation.moveWSCursor_ = function (xDirection, yDirection) {
  ...
  const newX = xDirection * Blockly.navigation.WS_MOVE_DISTANCE + wsCoord.x;
  const newY = yDirection * Blockly.navigation.WS_MOVE_DISTANCE + wsCoord.y;
